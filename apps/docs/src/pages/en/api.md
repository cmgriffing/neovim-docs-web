...
via [RPC|, |Lua| and VimL (|eval-api](#RPC|, |Lua| and VimL (|eval-api)).
...
See [channel-intro](#channel-intro) for various ways to open a channel. Channel-opening
...
Nvim creates a default RPC socket at [startup|, given by |v:servername](#startup|, given by |v:servername). To
start with a TCP/IP socket instead, use [--listen](#--listen) with a TCP-style address:
```
nvim --listen 127.0.0.1:6666
More endpoints can be started with [serverstart()](#serverstart()).
...
[api-client](#api-client). Here's a Ruby script that prints "hello world!" in the current
Nvim instance:
```
#!/usr/bin/env ruby
...
You can also embed Nvim via [jobstart()|, and communicate using |rpcrequest()](#jobstart()|, and communicate using |rpcrequest())
and [rpcnotify()](#rpcnotify()):
```
...
in the [api-metadata](#api-metadata) `types` key are stable: they will not change and are
thus forward-compatible.
```
EXT Type      C type                                  Data
------------------------------------------------------------------------
Buffer        enum value kObjectTypeBuffer            [bufnr()](#bufnr())
Window        enum value kObjectTypeWindow            [window-ID](#window-ID)
...
Exception: the following API functions use [extmarks](#extmarks) indexing (0-based
...
user input in a "modal" fashion (e.g. the [hit-enter-prompt](#hit-enter-prompt)), the request
will block.  Non-deferred (fast) functions such as [nvim_get_mode()](#nvim_get_mode()) and
[nvim_input()](#nvim_input()) are served immediately (i.e. without waiting in the input
queue).  Lua code can use [vim.in_fast_event()](#vim.in_fast_event()) to detect a fast context.
...
- functions               API function signatures, containing [api-types](#api-types) info
describing the return value and parameters.
- ui_events               [UI](#UI) event signatures
- ui_options              Supported [ui-option](#ui-option)s
...
a Buffer instance. [dev-api](#dev-api)
...
1. Connect to a running Nvim instance and call [nvim_get_api_info()](#nvim_get_api_info()) via
msgpack-RPC. This is best for clients written in dynamic languages which
can define functions at runtime.

2. Start Nvim with [--api-info](#--api-info). Useful for statically-compiled clients.
...
3. Use the [api_info()](#api_info()) Vimscript function.
```
:lua print(vim.inspect(vim.fn.api_info()))

```
     Example using [filter()](#filter()) to exclude non-deprecated API functions:
```
...
- Clients call functions like those described at [api-global](#api-global).
- Clients can subscribe to [ui-events|, |api-buffer-updates](#ui-events|, |api-buffer-updates), etc.
...
(Clients can dynamically check `api_prerelease`, etc. [api-metadata](#api-metadata))
...
This is similar to [TextChanged](#TextChanged) but more powerful and granular.

Call [nvim_buf_attach()](#nvim_buf_attach()) to receive these events on the channel:
...
When {changedtick} is [v:null](#v:null) this means the screen lines (display)
...
{changedtick} value of [b:changedtick](#b:changedtick) for the buffer. If you send an
API command back to nvim you can check the value of [b:changedtick](#b:changedtick) as
...
current change was chunked into multiple [nvim_buf_lines_event](#nvim_buf_lines_event)
...
When [b:changedtick](#b:changedtick) was incremented but no text was changed. Relevant for
undo/redo.

Properties:~
{buf} API buffer handle (buffer number)
{changedtick} new value of [b:changedtick](#b:changedtick) for the buffer
...
[nvim_buf_detach()](#nvim_buf_detach()) or implicitly in these cases:
- Buffer was [abandon](#abandon)ed and 'hidden' is not set.
- Buffer was reloaded, e.g. with [:edit](#:edit) or an external change triggered
[:checktime](#:checktime) or 'autoread'.
...
Calling [nvim_buf_attach()](#nvim_buf_attach()) with send_buffer=true on an empty buffer, emits:
```
...
User selects lines 3-5 using [linewise-visual](#linewise-visual) mode and then types "p" to
...
[textlock](#textlock) prevents changing buffer contents and window layout (use
[vim.schedule()](#vim.schedule()) to defer such operations to the main loop instead).

[nvim_buf_attach()](#nvim_buf_attach()) will take keyword args for the callbacks. "on_lines" will
...
`utf_sizes` is set to true in [nvim_buf_attach()](#nvim_buf_attach()) keyword args, then the
UTF-32 and UTF-16 sizes of the deleted region is also passed as additional
arguments {old_utf32_size} and {old_utf16_size}.

"on_changedtick" is invoked when [b:changedtick](#b:changedtick) was incremented but no text
...
callbacks attached with the same [nvim_buf_attach()](#nvim_buf_attach()) call.


## <a id="api-highlights" class="section-title" href="#api-highlights">Buffer Highlighting</a> 

Nvim allows plugins to add position-based highlights to buffers. This is
similar to [matchaddpos()](#matchaddpos()) but with some key differences. The added highlights
...
Highlights are registered using the [nvim_buf_add_highlight()](#nvim_buf_add_highlight()) function. If an
external highlighter plugin wants to add many highlights in a batch,
performance can be improved by calling [nvim_buf_add_highlight()](#nvim_buf_add_highlight()) as an
asynchronous notification, after first (synchronously) requesting a source id.

Example using the Python API client ([pynvim](#pynvim)):
```
...
src_id (this is the default in python). Use [nvim_buf_clear_namespace()](#nvim_buf_clear_namespace()) to
...
[api-window](#api-window) calls, and most window options (except 'statusline').

Two ways to create a floating window:
- [nvim_open_win()| creates a new window (needs a buffer, see |nvim_create_buf()](#nvim_open_win()| creates a new window (needs a buffer, see |nvim_create_buf()))
- [nvim_win_set_config()](#nvim_win_set_config()) reconfigures a normal window into a float

To close it use [nvim_win_close()| or a command such as |:close](#nvim_win_close()| or a command such as |:close).
...
[api-highlights|). The |hl-NormalFloat](#api-highlights|). The |hl-NormalFloat) group highlights normal text;
'winhighlight' can be used as usual to override groups locally. Floats inherit
options from the current window; specify `style=minimal` in [nvim_open_win()](#nvim_open_win())
...
The output of [:mksession](#:mksession) does not include commands for restoring floating
windows.
...
that needs to track a logical location in the buffer over time. [api-indexing](#api-indexing)
...
f o o[b a r      line (](#b a r      line () = cursor)
3           extmark

f o o z[b a r    line (](#b a r    line () = cursor)
...
f o o z b a r[   line (](#   line () = cursor)
7   extmark

f o o z b a r    first line
extmarks (none present)
[                second line (](#                second line () = cursor)
...
[api-indexing](#api-indexing) Passing id=0 creates a new mark and returns the id:
```
...
We can get all marks in a buffer by [namespace](#namespace) (or by a range):
```
...
use [nvim_buf_del_extmark()](#nvim_buf_del_extmark()). Deleting "x" in our example:
```
...
See [nvim_buf_set_extmark()](#nvim_buf_set_extmark())
...
processing which have such side effects, e.g. [:sleep](#:sleep) may wake
timers).
...
process. For the stdio channel [channel-stdio](#channel-stdio), it writes to Nvim's
stdout. For an internal terminal instance ([nvim_open_term()](#nvim_open_term())) it writes
directly to terminal output. See [channel-bytes](#channel-bytes) for more information.
...
[vim.rpcnotify()| and |vim.rpcrequest()](#vim.rpcnotify()| and |vim.rpcrequest()) instead.

Attributes: ~
[RPC](#RPC) only
Lua [vim.api](#vim.api) only
...
• {scratch}  Creates a "throwaway" [scratch-buffer](#scratch-buffer) for temporary work
...
Attributes: ~
not allowed when [textlock](#textlock) is active

### <a id="nvim_del_keymap()" class="section-title" href="#nvim_del_keymap()">nvim_del_keymap({mode}, {lhs})</a>
Unmaps a global [mapping](#mapping) for the given mode.

To unmap a buffer-local mapping, use [nvim_buf_del_keymap()](#nvim_buf_del_keymap()).

See also: ~
[nvim_set_keymap()](#nvim_set_keymap())

### <a id="nvim_del_mark()" class="section-title" href="#nvim_del_mark()">nvim_del_mark({name})</a>
Deletes an uppercase/file named mark. See [mark-motions](#mark-motions).
...
• {history}  if true, add to [message-history](#message-history).
...
• winid: (number) [window-ID](#window-ID) of the window to use as context
for statusline.
...
true. Each element of the array is a [Dictionary](#Dictionary) with these keys:
...
flags. This is a blocking call, unlike [nvim_input()](#nvim_input()).

On execution error: does not fail, but updates v:errmsg.

To input sequences like <C-o> use [nvim_replace_termcodes()](#nvim_replace_termcodes()) (typically
with escape_ks=false) to replace [keycodes](#keycodes), then pass the result to
nvim_feedkeys().
...
Parameters: ~
• {keys}       to be typed
• {mode}       behavior flags, see [feedkeys()](#feedkeys())
• {escape_ks}  If true, escape K_SPECIAL bytes in `keys` This should be
false if you already used [nvim_replace_termcodes()](#nvim_replace_termcodes()), and
true otherwise.
...
1 is the [api-metadata](#api-metadata) map (Dictionary).
...
• "terminal" [terminal](#terminal) instance interprets ASCII sequences.
• "rpc" [RPC](#RPC) communication on the channel is active.
...
• "buffer" (optional) Buffer with connected [terminal](#terminal) instance.
• "client" (optional) Info about the peer (client on the other end of
the RPC channel), if provided by it via [nvim_set_client_info()](#nvim_set_client_info()).

### <a id="nvim_get_color_by_name()" class="section-title" href="#nvim_get_color_by_name()">nvim_get_color_by_name({name})</a>
Returns the 24-bit RGB value of a [nvim_get_color_map()](#nvim_get_color_map()) color name or
...
Parameters: ~
• {opts}  Optional parameters.
• types: List of [context-types](#context-types) ("regs", "jumps", "bufs",
"gvars", …) to gather, or empty for "all".

Return: ~
map of global [context](#context).
...
Gets a highlight definition by id. [hlID()](#hlID())

Parameters: ~
• {hl_id}  Highlight id as returned by [hlID()](#hlID())
...
similar to [hlID()](#hlID()), but allocates a new ID if not present.

### <a id="nvim_get_keymap()" class="section-title" href="#nvim_get_keymap()">nvim_get_keymap({mode})</a>
Gets a list of global (non-buffer-local) [mapping](#mapping) definitions.

Parameters: ~
• {mode}  Mode short-name ("n", "i", "v", ...)

Return: ~
Array of [maparg()](#maparg())-like dictionaries describing mappings. The
...
the uppercase/file named mark. See [mark-motions](#mark-motions).

Marks are (1,0)-indexed. [api-indexing](#api-indexing)
...
Gets the current mode. [mode()](#mode()) "blocking" is true if Nvim is waiting for
input.
...
Queues raw user-input. Unlike [nvim_feedkeys()](#nvim_feedkeys()), this uses a low-level
...
Note:
[keycodes](#keycodes) like <CR> are translated, so "<" is special. To input a
literal "<", send <LT>.

Note:
For mouse events use [nvim_input_mouse()](#nvim_input_mouse()). The pseudokey form
"<LeftMouse><col,row>" is deprecated since [api-level](#api-level) 6.
...
[nvim_input()](#nvim_input()) has the same limitation.
...
• {grid}      Grid number if the client uses [ui-multigrid](#ui-multigrid), else 0.
...
[nvim_buf_is_loaded()](#nvim_buf_is_loaded()) to check if a buffer is loaded.
...
specified at [nvim_get_chan_info()](#nvim_get_chan_info()).
...
• "rgb" true if the UI uses RGB colors (false implies [cterm-colors](#cterm-colors))
• "ext_..." Requested UI extensions, see [ui-option](#ui-option)
...
Sets the current editor state from the given [context](#context) map.

Parameters: ~
• {dict}  [Context](#Context) map.
...
floating display, first create an empty buffer using [nvim_create_buf()](#nvim_create_buf()),
then display it using [nvim_open_win()](#nvim_open_win()), and then call this function. Then
[nvim_chan_send()](#nvim_chan_send()) can be called immediately to process sequences in a
...
is sent as a "\r", not as a "\n". [textlock](#textlock) applies. It
is possible to call [nvim_chan_send()](#nvim_chan_send()) directly in the
...
Sets redo/undo. Faster than [nvim_input()](#nvim_input()). Lines break at LF ("\n").
...
Compare [:put| and |p](#:put| and |p) which are always linewise.

Attributes: ~
not allowed when [textlock](#textlock) is active

Parameters: ~
• {lines}   [readfile()|-style list of lines. |channel-lines](#readfile()|-style list of lines. |channel-lines)
• {type}    Edit behavior: any [getregtype()](#getregtype()) result, or:
• "b" [blockwise-visual](#blockwise-visual) mode (may include width, e.g. "b3")
• "c" [charwise](#charwise) mode
• "l" [linewise](#linewise) mode
• "" guess by contents, see [setreg()](#setreg())
• {after}   If true insert after cursor (like [p](#p)), or before (like
[P](#P)).
• {follow}  If true place cursor at end of inserted text.

### <a id="nvim_replace_termcodes()" class="section-title" href="#nvim_replace_termcodes()">Note:</a>
nvim_replace_termcodes({str}, {from_part}, {do_lt}, {special})
Replaces terminal codes and [keycodes](#keycodes) (<CR>, <Esc>, ...) in a string with
...
• {special}    Replace [keycodes](#keycodes), e.g. <CR> becomes a "\r" char.
...
If neither [ins-completion| nor |cmdline-completion](#ins-completion| nor |cmdline-completion) popup menu is active
this API call is silently ignored. Useful for an external UI using
[ui-popupmenu](#ui-popupmenu) to control the popup menu with the mouse. Can also be used
in a mapping; use <Cmd> [:map-cmd](#:map-cmd) or a Lua mapping to ensure the mapping
doesn't end completion mode.

Parameters: ~
• {item}    Index (zero-based) of the item to select. Value of -1
selects nothing and restores the original text.
• {insert}  For [ins-completion](#ins-completion), whether the selection should be
inserted in the buffer. Ignored for [cmdline-completion](#cmdline-completion).
...
Parameters: ~
• {ns_id}  Namespace id for this highlight [nvim_create_namespace()](#nvim_create_namespace()).
Use 0 to set a highlight group globally [:highlight](#:highlight).
...
[:hi-link](#:hi-link).
• default: Don't override existing definition [:hi-default](#:hi-default)
• ctermfg: Sets foreground of cterm color [ctermfg](#ctermfg)
• ctermbg: Sets background of cterm color [ctermbg](#ctermbg)
• cterm: cterm attribute map, like [highlight-args](#highlight-args). If not
...
see [nvim_win_set_hl_ns()](#nvim_win_set_hl_ns()).
...
[nvim_set_decoration_provider()](#nvim_set_decoration_provider()) on_win and on_line callbacks, which are
...
Sets a global [mapping](#mapping) for the given mode.

To set a buffer-local mapping, use [nvim_buf_set_keymap()](#nvim_buf_set_keymap()).

Unlike [:map](#:map), leading/trailing whitespace is accepted as part of the
{lhs} or {rhs}. Empty {rhs} is [<Nop>|. |keycodes](#<Nop>|. |keycodes) are replaced as usual.
...
"!" for [:map!|, or empty string for |:map](#:map!|, or empty string for |:map).
• {lhs}   Left-hand-side [{lhs}](#{lhs}) of the mapping.
• {rhs}   Right-hand-side [{rhs}](#{rhs}) of the mapping.
• {opts}  Optional parameters map: keys are [:map-arguments](#:map-arguments), values are
booleans (default false). Accepts all [:map-arguments](#:map-arguments) as keys
excluding [<buffer>| but including |:noremap](#<buffer>| but including |:noremap) and "desc".
...
string (see [nvim_replace_termcodes()](#nvim_replace_termcodes())), and a Lua callback
...
Calls a VimL [Dictionary-function](#Dictionary-function) with the given arguments.

On execution error: fails with VimL error, updates v:errmsg.

Parameters: ~
• {dict}  Dictionary, or String evaluating to a VimL [self](#self) dict
...
Prefer using [nvim_cmd()| or |nvim_exec()](#nvim_cmd()| or |nvim_exec()) over this. To evaluate multiple
lines of Vim script or an Ex command directly, use [nvim_exec()](#nvim_exec()). To
construct an Ex command using a structured format and then execute it, use
[nvim_cmd()](#nvim_cmd()). To modify an Ex command before evaluating it, use
[nvim_parse_cmd()| in conjunction with |nvim_cmd()](#nvim_parse_cmd()| in conjunction with |nvim_cmd()).

Parameters: ~
• {command}  Ex command string

### <a id="nvim_eval()" class="section-title" href="#nvim_eval()">nvim_eval({expr})</a>
Evaluates a VimL [expression](#expression). Dictionaries and Lists are recursively
expanded.
...
Unlike [nvim_command()](#nvim_command()) this function supports heredocs, script-scope
(s:), etc.
...
Create a new user command [user-commands](#user-commands) in the given buffer.
...
Only commands created with [:command-buffer](#:command-buffer) or
[nvim_buf_create_user_command()](#nvim_buf_create_user_command()) can be deleted with this function.
...
Gets a map of buffer-local [user-commands](#user-commands).
...
Unlike [nvim_command()](#nvim_command()) this command takes a structured Dictionary instead
...
support it in order to make their usage simpler with [vim.cmd()](#vim.cmd()). For
...
same values as the return value of [nvim_parse_cmd()](#nvim_parse_cmd()) except
...
Create a new user command [user-commands](#user-commands)
...
0, 1, or 2 [<range>](#<range>)
• count: (number) Any count supplied [<count>](#<count>)
• reg: (string) The optional register, if specified [<reg>](#<reg>)
• mods: (string) Command modifiers, if any [<mods>](#<mods>)
• smods: (table) Command modifiers in a structured format.
Has the same structure as the "mods" key of
[nvim_parse_cmd()](#nvim_parse_cmd()).
• {opts}     Optional command attributes. See [command-attributes](#command-attributes) for
more details. To use boolean attributes (such as
[:command-bang| or |:command-bar](#:command-bang| or |:command-bar)) set the value to "true".
In addition to the string options listed in
[:command-complete](#:command-complete), the "complete" key also accepts a Lua
function which works like the "customlist" completion mode
[:command-completion-customlist](#:command-completion-customlist). Additional parameters:
...
Currently only [user-commands](#user-commands) are supported, not builtin Ex commands.
...
• range: (array) (optional) Command range ([<line1>| |<line2>](#<line1>| |<line2>)).
...
specified.
• count: (number) (optional) Command [<count>](#<count>). Omitted if command
cannot take a count.
• reg: (string) (optional) Command [<register>](#<register>). Omitted if command
cannot take a register.
• bang: (boolean) Whether command contains a [<bang>](#<bang>) (!) modifier.
• args: (array) Command arguments.
• addr: (string) Value of [:command-addr](#:command-addr). Uses short name.
• nargs: (string) Value of [:command-nargs](#:command-nargs).
• nextcmd: (string) Next command if there are multiple commands
separated by a [:bar](#:bar). Empty if there isn't a next command.
...
• mods: (dictionary) [:command-modifiers](#:command-modifiers).
• filter: (dictionary) [:filter](#:filter).
• pattern: (string) Filter pattern. Empty string if there is no
filter.
• force: (boolean) Whether filter is inverted or not.

• silent: (boolean) [:silent](#:silent).
• emsg_silent: (boolean) [:silent!](#:silent!).
• unsilent: (boolean) [:unsilent](#:unsilent).
• sandbox: (boolean) [:sandbox](#:sandbox).
• noautocmd: (boolean) [:noautocmd](#:noautocmd).
• browse: (boolean) [:browse](#:browse).
• confirm: (boolean) [:confirm](#:confirm).
• hide: (boolean) [:hide](#:hide).
• horizontal: (boolean) [:horizontal](#:horizontal).
• keepalt: (boolean) [:keepalt](#:keepalt).
• keepjumps: (boolean) [:keepjumps](#:keepjumps).
• keepmarks: (boolean) [:keepmarks](#:keepmarks).
• keeppatterns: (boolean) [:keeppatterns](#:keeppatterns).
• lockmarks: (boolean) [:lockmarks](#:lockmarks).
• noswapfile: (boolean) [:noswapfile](#:noswapfile).
• tab: (integer) [:tab](#:tab). -1 when omitted.
• verbose: (integer) [:verbose](#:verbose). -1 when omitted.
...
one of:
• "aboveleft": [:aboveleft](#:aboveleft).
• "belowright": [:belowright](#:belowright).
...
dictionaries as detailed at [nvim_get_option_info()](#nvim_get_option_info()).
...
[:set](#:set): the local value of an option is returned if it exists; otherwise,
...
• {name}  Option name
• {opts}  Optional parameters
• scope: One of "global" or "local". Analogous to [:setglobal](#:setglobal)
and [:setlocal](#:setlocal), respectively.
• win: [window-ID](#window-ID). Used for getting window local options.
...
[:set](#:set): for global-local options, both the global and local value are set
...
[:setglobal| and |:setlocal](#:setglobal| and |:setlocal), respectively.
• win: [window-ID](#window-ID). Used for setting window local option.
...
For more information on buffers, see [buffers](#buffers).

Unloaded Buffers:~

Buffers may be unloaded by the [:bunload](#:bunload) command or the buffer's
['bufhidden'](#'bufhidden') option. When a buffer is unloaded its file contents are
...
methods such as [nvim_buf_get_lines()| and |nvim_buf_line_count()](#nvim_buf_get_lines()| and |nvim_buf_line_count()) will be
affected.

You can use [nvim_buf_is_loaded()| or |nvim_buf_line_count()](#nvim_buf_is_loaded()| or |nvim_buf_line_count()) to check
...
buffer/window currently, like [termopen()](#termopen()).
...
Unmaps a buffer-local [mapping](#mapping) for the given mode.
...
Deletes a named mark in the buffer. See [mark-motions](#mark-motions).
...
Deletes the buffer. See [:bwipeout](#:bwipeout)
...
• unload: Unloaded only, do not delete. See [:bunload](#:bunload)
...
[api-lua-detach](#api-lua-detach) for detaching Lua callbacks
...
Gets a list of buffer-local [mapping](#mapping) definitions.
...
Returns the byte offset of a line (0-indexed). [api-indexing](#api-indexing)
...
Unlike [line2byte()](#line2byte()), throws error for out-of-bounds indexing. Returns -1
for unloaded buffer.
...
This differs from [nvim_buf_get_lines()](#nvim_buf_get_lines()) in that it allows retrieving only
portions of a line.

Indexing is zero-based. Row indices are end-inclusive, and column indices
are end-exclusive.

Prefer [nvim_buf_get_lines()](#nvim_buf_get_lines()) when retrieving entire lines.
...
Checks if a buffer is valid and loaded. See [api-buffer](#api-buffer) for more info
...
Note:
Even if a buffer is valid it may have been unloaded. See [api-buffer](#api-buffer)
...
Return: ~
Line count, or 0 for unloaded buffer. [api-buffer](#api-buffer)

### <a id="nvim_buf_set_keymap()" class="section-title" href="#nvim_buf_set_keymap()">Note:</a>
nvim_buf_set_keymap({buffer}, {mode}, {lhs}, {rhs}, {*opts})
Sets a buffer-local [mapping](#mapping) for the given mode.
...
file/uppercase, visual, last change, etc. See [mark-motions](#mark-motions).
...
This is recommended over [nvim_buf_set_lines()](#nvim_buf_set_lines()) when only modifying parts
...
Prefer [nvim_buf_set_lines()](#nvim_buf_set_lines()) if you are only adding or deleting entire
lines.
...
a buffer. Unlike [matchaddpos()](#matchaddpos()) highlights follow changes to line
numbering (as lines are inserted/removed above the highlighted line), like
signs and marks do.

Namespaces are used for batch deletion/updating of a set of highlights. To
create a namespace, use [nvim_create_namespace()](#nvim_create_namespace()) which returns a
namespace id. Pass it in to this function as `ns_id` to add highlights to
the namespace. All highlights in the same namespace can then be cleared
with single call to [nvim_buf_clear_namespace()](#nvim_buf_clear_namespace()). If the highlight never
...
[nvim_create_namespace()](#nvim_create_namespace()) to create a new empty namespace.
...
Lines are 0-indexed. [api-indexing](#api-indexing) To clear the namespace in the entire
...
Parameters: ~
• {buffer}  Buffer handle, or 0 for current buffer
• {ns_id}   Namespace id from [nvim_create_namespace()](#nvim_create_namespace())
• {id}      Extmark id
...
Gets extmarks in "traversal order" from a [charwise](#charwise) region defined by
buffer positions (inclusive, 0-indexed [api-indexing](#api-indexing)).
...
(whose position defines the bound). [api-indexing](#api-indexing)
...
• {buffer}  Buffer handle, or 0 for current buffer
• {ns_id}   Namespace id from [nvim_create_namespace()](#nvim_create_namespace())
• {line}    Line where to place the mark, 0-based. [api-indexing](#api-indexing)
• {col}     Column where to place the mark, 0-based. [api-indexing](#api-indexing)
• {opts}    Optional parameters.
...
using [nvim_get_hl_id_by_name()](#nvim_get_hl_id_by_name()).
...
• ephemeral : for use with [nvim_set_decoration_provider()](#nvim_set_decoration_provider())
...
character. Enable concealing similar to [:syn-conceal](#:syn-conceal).
When a character is supplied it is used as [:syn-cchar](#:syn-cchar).
"hl_group" is used as highlight for the cchar if provided,
otherwise it defaults to [hl-Conceal](#hl-Conceal).
...
[nvim_buf_set_extmark()](#nvim_buf_set_extmark()) can be called to add marks on a per-window or
...
Closes the window (like [:close| with a |window-ID](#:close| with a |window-ID)).
...
positions). [api-indexing](#api-indexing)
...
Closes the window and hide the buffer it contains (like [:hide](#:hide) with a
[window-ID](#window-ID)).

Like [:hide](#:hide) the buffer becomes hidden unless another window is editing
it, or 'bufhidden' is `unload`, `delete` or `wipe` as opposed to [:close](#:close)
or [nvim_win_close()](#nvim_win_close()), which will close the buffer.
...
Sets the (1,0)-indexed cursor position in the window. [api-indexing](#api-indexing) This
...
the [ui-multigrid](#ui-multigrid) extension. External windows are only supported with
multigrid GUIs, and are displayed as separate top-level windows.

For a general overview of floats, see [api-floatwin](#api-floatwin).
...
• win: [window-ID](#window-ID) for relative="win".
...
entered by [nvim_set_current_win()](#nvim_set_current_win()).
...
char, and clearing the [hl-EndOfBuffer](#hl-EndOfBuffer) region in
'winhighlight'.
...
events such as [BufEnter|, |BufLeave| or |BufWinEnter](#BufEnter|, |BufLeave| or |BufWinEnter) may
...
The returned value may be given to [nvim_open_win()](#nvim_open_win()).
...
Return: ~
Map defining the window configuration, see [nvim_open_win()](#nvim_open_win())
...
Parameters: ~
• {window}  Window handle, or 0 for current window
• {config}  Map defining the window configuration, see [nvim_open_win()](#nvim_open_win())
...
• buffer: (bufnr)
• clear only [autocmd-buflocal](#autocmd-buflocal) autocommands.
...
Create or get an autocommand group [autocmd-groups](#autocmd-groups).
...
commands if the group already exists [autocmd-groups](#autocmd-groups).
...
Create an [autocommand](#autocommand)
...
```
### <a id="pattern = ".py"" class="section-title" href="#pattern = ".py"">Note:</a>
...
match against [autocmd-pattern](#autocmd-pattern).
• buffer (integer) optional: buffer number for buffer local
autocommands [autocmd-buflocal](#autocmd-buflocal). Cannot be used with
{pattern}.
...
autocommand [autocmd-events](#autocmd-events)
• group: (number|nil) the autocommand group id, if it
exists
• match: (string) the expanded value of [<amatch>](#<amatch>)
• buf: (number) the expanded value of [<abuf>](#<abuf>)
• file: (string) the expanded value of [<afile>](#<afile>)
...
autocommand only once [autocmd-once](#autocmd-once).
• nested (boolean) optional: defaults to false. Run nested
autocommands [autocmd-nested](#autocmd-nested).
...
To get a group id one can use [nvim_get_autocmds()](#nvim_get_autocmds()).

NOTE: behavior differs from [:augroup-delete](#:augroup-delete). When deleting a group,
...
• {opts}   Dictionary of autocommand options:
• group (string|integer) optional: the autocommand group name
or id to match against. [autocmd-groups](#autocmd-groups).
### <a id="• pattern (string[array) optional: defaults to """ class="section-title" href="#• pattern (string](#array) optional: defaults to """ class="section-title" href="#• pattern (string)array) optional: defaults to """>Note:</a>
[autocmd-pattern](#autocmd-pattern). Cannot be used with {buffer}.
• buffer (integer) optional: buffer number
[autocmd-buflocal](#autocmd-buflocal). Cannot be used with {pattern}.
• modeline (bool) optional: defaults to true. Process the
modeline after the autocommands [<nomodeline>](#<nomodeline>).
• data (any): arbitrary data to send to the autocommand
callback. See [nvim_create_autocmd()](#nvim_create_autocmd()) for details.
...
local autocommands [autocmd-buflocal](#autocmd-buflocal). Cannot be used with
{pattern}
...
• callback (function[string](#string)nil): Lua function or name of a Vim script
...
buffer local [autocmd-buffer-local](#autocmd-buffer-local):
...
Entry point of all UI clients. Allows [--embed](#--embed) to continue startup.
Implies that the client is ready to show the UI. Adds the client to the
list of UIs. [nvim_list_uis()](#nvim_list_uis())
...
• {options}  [ui-option](#ui-option) map

### <a id="nvim_ui_detach()" class="section-title" href="#nvim_ui_detach()">nvim_ui_detach()</a>
Deactivates UI events on the channel.

Removes the client from the list of UIs. [nvim_list_uis()](#nvim_list_uis())
...
[nvim_ui_pum_set_height()](#nvim_ui_pum_set_height()), which sets the number of visible items in the

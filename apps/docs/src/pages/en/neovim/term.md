...
(except in [--embed](undefined#--embed) and [--headless](undefined#--headless) modes). If that information is wrong,
...
Nvim guesses the terminal type when it starts (except in [--embed](undefined#--embed) and
[--headless](undefined#--headless) modes). The |$TERM| environment variable is the primary hint that
...
unlike most other environment variables.
  ...
  For this terminal           Set $TERM to                  [builtin-terms](undefined#builtin-terms)
  -------------------------------------------------------------------------
...
If a [terminfo](undefined#terminfo) database is not available or there is no entry for the current
...
be mapped to the builtin "putty" entry. See also [tui-colors](undefined#tui-colors).
...
[terminfo](undefined#terminfo) is used first, and CSI sequences not in [terminfo](undefined#terminfo) (including
...
Nvim uses 256 colours by default, ignoring [terminfo](undefined#terminfo) for most terminal types,
...
It uses the "setrgbf" and "setrgbb" [terminfo](undefined#terminfo) extensions (proposed by Rüdiger
...
extension pioneered by dtterm.  [terminfo](undefined#terminfo) does not have a flag for this
...
it.  It uses the same [terminfo](undefined#terminfo) extensions that were pioneered by tmux for
this: "Ss" and "Se".
...
- from the [terminfo](undefined#terminfo) entries "lines" and "columns"
...
See the "Options" chapter [options](undefined#options).
...
[CTRL-O](undefined#CTRL-O) and [CTRL-I](undefined#CTRL-I).

...
If no files have been marked via [netrw-mf](#netrw-mf):

The "a" map allows the user to cycle through the three hiding modes.

The [g:netrw_list_hide](#g:netrw_list_hide) variable holds a comma delimited list of patterns
based on regular expressions (ex. ^.*\.obj$,^\.) which specify the hiding list.
(also see [netrw-ctrl-h](#netrw-ctrl-h))  To set the hiding list, use the <c-h> map.  As an
...
If files have been marked using [netrw-mf](#netrw-mf), then this command will:
...
Associated setting variables: [g:netrw_list_hide|  |g:netrw_hide](#g:netrw_list_hide|  |g:netrw_hide)
Associated topics: [netrw-a| |netrw-ctrl-h| |netrw-mh](#netrw-a| |netrw-ctrl-h| |netrw-mh)
...
[g:netrw_list_hide](#g:netrw_list_hide) automatically hides all git-ignored files.
...
To use this function, simply assign its output to [g:netrw_list_hide](#g:netrw_list_hide) option.
```
...
[netrw-mb](#netrw-mb) how to make a bookmark
[netrw-mB](#netrw-mB) how to delete bookmarks
[netrw-u](#netrw-u)  change to a predecessor directory via the history stack
[netrw-U](#netrw-U)  change to a successor   directory via the history stack
...
Related topics: [netrw-D](#netrw-D)
Associated setting variables:	[g:netrw_localmkdir|   |g:netrw_mkdir_cmd](#g:netrw_localmkdir|   |g:netrw_mkdir_cmd)
[g:netrw_remote_mkdir| |netrw-%](#g:netrw_remote_mkdir| |netrw-%)


MAKING THE BROWSING DIRECTORY THE CURRENT DIRECTORY	*netrw-cd* {{{2

By default, [g:netrw_keepdir](#g:netrw_keepdir) is 1.  This setting means that the current
...
[netrw-cd| : This map's name was changed from "c" to cd (see |netrw-cd](#netrw-cd| : This map's name was changed from "c" to cd (see |netrw-cd)).
This change was done to allow for [netrw-cb| and |netrw-cB](#netrw-cb| and |netrw-cB) maps.

Associated setting variable: [g:netrw_keepdir](#g:netrw_keepdir)

### <a id="netrw-:MF	netrw-mf {{{2" class="section-title" href="#netrw-:MF	netrw-mf {{{2">Marking Files</a>
(also see [netrw-mr](#netrw-mr))
...
(Note that :MF uses [<f-args>](#<f-args>) to break the line
at spaces)

* Mark files using the [argument-list| (|netrw-mA](#argument-list| (|netrw-mA))

* Mark files based upon a [location-list| (|netrw-qL](#location-list| (|netrw-qL))

* Mark files based upon the quickfix list ([netrw-qF](#netrw-qF))
([quickfix-error-lists](#quickfix-error-lists))

The following netrw maps make use of marked files:

[netrw-a](#netrw-a)	Hide marked files/directories
[netrw-D](#netrw-D)	Delete marked files/directories
[netrw-ma|	Move marked files' names to |arglist](#netrw-ma|	Move marked files' names to |arglist)
[netrw-mA|	Move |arglist](#netrw-mA|	Move |arglist) filenames to marked file list
[netrw-mb](#netrw-mb)	Append marked files to bookmarks
[netrw-mB](#netrw-mB)	Delete marked files from bookmarks
[netrw-mc](#netrw-mc)	Copy marked files to target
[netrw-md](#netrw-md)	Apply vimdiff to marked files
[netrw-me](#netrw-me)	Edit marked files
[netrw-mF](#netrw-mF)	Unmark marked files
[netrw-mg](#netrw-mg)	Apply vimgrep to marked files
[netrw-mm](#netrw-mm)	Move marked files to target
[netrw-mp](#netrw-mp)	Print marked files
[netrw-ms](#netrw-ms)	Netrw will source marked files
[netrw-mt|	Set target for |netrw-mm| and |netrw-mc](#netrw-mt|	Set target for |netrw-mm| and |netrw-mc)
[netrw-mT](#netrw-mT)	Generate tags using marked files
[netrw-mv](#netrw-mv)	Apply vim command to marked files
[netrw-mx](#netrw-mx)	Apply shell command to marked files
[netrw-mX](#netrw-mX)	Apply shell command to marked files, en bloc
...
[group-name](#group-name)).  You may change the highlighting group by putting something
like
```
...
leftmouse.  (see [netrw-mouse](#netrw-mouse))
...
Known Problem: if one is using tree mode ([g:netrw_liststyle](#g:netrw_liststyle)) and several
...
[markfilelist](#markfilelist), however, will only have the selected file in it.  This problem
is unlikely to be fixed.


### <a id="netrw-mF {{{2" class="section-title" href="#netrw-mF {{{2">Unmarking Files</a>
(also see [netrw-mf|, |netrw-mu](#netrw-mf|, |netrw-mu))

The "mF" command will unmark all files in the current buffer.  One may also use
mf ([netrw-mf](#netrw-mf)) on a specific, already marked, file to unmark just that file.

### <a id="netrw-qL {{{2" class="section-title" href="#netrw-qL {{{2">Marking Files by Location List</a>
(also see [netrw-mf](#netrw-mf))

One may convert [location-list](#location-list)s into a marked file list using "qL".
You may then proceed with commands such as me ([netrw-me](#netrw-me)) to edit them.


### <a id="netrw-qF {{{2" class="section-title" href="#netrw-qF {{{2">Marking Files by Quickfix List</a>
(also see [netrw-mf](#netrw-mf))

One may convert [quickfix-error-lists](#quickfix-error-lists) into a marked file list using "qF".
You may then proceed with commands such as me ([netrw-me](#netrw-me)) to edit them.
Quickfix error lists are generated, for example, by calls to [:vimgrep](#:vimgrep).
...
as *.c$ (see [glob()](#glob())).  For remote systems, glob() doesn't work -- so netrw
converts "*" into ".*" (see [regexp](#regexp)) and marks files based on that.  In the
future I may make it possible to use [regexp](#regexp)s instead of glob()-style
expressions (yet-another-option).

See [cmdline-window](#cmdline-window) for directions on more on how to edit the regular
expression.


### <a id="netrw-mv" class="section-title" href="#netrw-mv">Marked Files, Arbitrary Vim Command</a>
(See [netrw-mf| and |netrw-mr](#netrw-mf| and |netrw-mr) for how to mark files)
...
wish used.  See [cmdline-window](#cmdline-window) for directions for more on how to edit the
command.
...
See Also: [netrw-cb| |netrw-cB| |netrw-qF| |argument-list| |:args](#netrw-cb| |netrw-cB| |netrw-qF| |argument-list| |:args)
...
See Also: [netrw-ma| |netrw-mA| |netrw-qF| |buffer-list| |:buffers](#netrw-ma| |netrw-mA| |netrw-qF| |buffer-list| |:buffers)
...
using the command specified by [g:netrw_compress](#g:netrw_compress); by default,
that's "gzip".

For decompression, netrw uses a [Dictionary](#Dictionary) of suffices and their
associated decompressing utilities; see [g:netrw_decompress](#g:netrw_decompress).

Remember that one can mark multiple files by regular expression
(see [netrw-mr](#netrw-mr)); this is particularly useful to facilitate compressing and
decompressing a large number of files.

Associated setting variables: [g:netrw_compress| |g:netrw_decompress](#g:netrw_compress| |g:netrw_decompress)

### <a id="netrw-mc {{{2" class="section-title" href="#netrw-mc {{{2">Marked Files: Copying</a>
(See [netrw-mf| and |netrw-mr](#netrw-mf| and |netrw-mr) for how to mark files)
(Uses the global marked file list)

Select a target directory with mt ([netrw-mt](#netrw-mt)).  Then change directory,
select file(s) (see [netrw-mf](#netrw-mf)), and press "mc".  The copy is done
from the current window (where one does the mf) to the target.

If one does not have a target directory set with [netrw-mt](#netrw-mt), then netrw
...
Associated setting variables:
[g:netrw_localcopycmd|		|g:netrw_localcopycmdopt](#g:netrw_localcopycmd|		|g:netrw_localcopycmdopt)
[g:netrw_localcopydircmd|	|g:netrw_localcopydircmdopt](#g:netrw_localcopydircmd|	|g:netrw_localcopydircmdopt)
...
The "me" command will place the marked files on the [arglist](#arglist) and commence
editing them.  One may return the to explorer window with [:Rexplore](#:Rexplore).
(use [:n| and |:p](#:n| and |:p) to edit next and previous files in the arglist)

### <a id="netrw-mg {{{2" class="section-title" href="#netrw-mg {{{2">Marked Files: Grep</a>
(See [netrw-mf| and |netrw-mr](#netrw-mf| and |netrw-mr) for how to mark files)
(uses the global marked file list)

The "mg" command will apply [:vimgrep](#:vimgrep) to the marked files.
...
With /pattern/, editing will start with the first item on the [quickfix](#quickfix) list
that vimgrep sets up (see [:copen|, |:cnext|, |:cprevious|, |:cclose|).  The |:vimgrep](#:copen|, |:cnext|, |:cprevious|, |:cclose|).  The |:vimgrep)
...
Related topics: [netrw-a| |g:netrw_list_hide](#netrw-a| |g:netrw_list_hide)
...
Select a target directory with mt ([netrw-mt](#netrw-mt)).  Then change directory,
select file(s) (see [netrw-mf](#netrw-mf)), and press "mm".  The move is done
from the current window (where one does the mf) to the target.

Associated setting variable: [g:netrw_localmovecmd| |g:netrw_ssh_cmd](#g:netrw_localmovecmd| |g:netrw_ssh_cmd)

### <a id="netrw-mp {{{2" class="section-title" href="#netrw-mp {{{2">Marked Files: Printing</a>
(See [netrw-mf| and |netrw-mr](#netrw-mf| and |netrw-mr) for how to mark files)
(uses the local marked file list)

When "mp" is used, netrw will apply the [:hardcopy](#:hardcopy) command to marked files.
...
With "ms", netrw will source the marked files (using vim's [:source](#:source) command)


### <a id="netrw-mt {{{2" class="section-title" href="#netrw-mt {{{2">Marked Files: Setting the Target Directory</a>
(See [netrw-mf| and |netrw-mr](#netrw-mf| and |netrw-mr) for how to mark files)

Set the marked file copy/move-to target (see [netrw-mc| and |netrw-mm](#netrw-mc| and |netrw-mm)):
...
This command uses [<q-args>](#<q-args>), so spaces in the directory name are
...
target is a script variable (see [s:var](#s:var)) and is shared between all netrw
...
The "mT" mapping will apply the command in [g:netrw_ctags](#g:netrw_ctags) (by default, it is
"ctags") to marked files.  For remote browsing, in order to create a tags file
netrw will use ssh (see [g:netrw_ssh_cmd](#g:netrw_ssh_cmd)), and so ssh must be available for
this to work on remote systems.  For your local system, see [ctags](#ctags) on how to
...
uses one of the go to tag actions ([tags](#tags)), the URL will be used by netrw to
edit the desired file and go to the tag.

Associated setting variables: [g:netrw_ctags| |g:netrw_ssh_cmd](#g:netrw_ctags| |g:netrw_ssh_cmd)
...
The [netrw-qb](#netrw-qb) map will give you a list of bookmarks (and history).
One may choose one of the bookmarks to become your marked file
target by using [count]Tb (default count: 1).

Related topics:
Copying files to target............................[netrw-mc](#netrw-mc)
Listing Bookmarks and History......................[netrw-qb](#netrw-qb)
Marked Files: Setting The Target Directory.........[netrw-mt](#netrw-mt)
Marked Files: Target Directory Using History.......[netrw-Th](#netrw-Th)
Marking Files......................................[netrw-mf](#netrw-mf)
Marking Files by Regular Expression................[netrw-mr](#netrw-mr)
Moving files to target.............................[netrw-mm](#netrw-mm)
...
The [netrw-qb](#netrw-qb) map will give you a list of history (and bookmarks).
...
(See [netrw-mf|, |netrw-mF](#netrw-mf|, |netrw-mF))
...
(if you're interested in the netrw file transfer settings, see [netrw-options](#netrw-options)
and [netrw-protocol](#netrw-protocol))

The <netrw.vim> browser provides settings in the form of variables which
you may modify; by placing these settings in your <.vimrc>, you may customize
your browsing preferences.  (see also: [netrw-settings](#netrw-settings))
```
---				-----------
Var				Explanation
---				-----------
### <a id="g:netrw_altfile" class="section-title" href="#g:netrw_altfile"><</a>
edited file.  Choose that by setting this
parameter to 1.
Others like [CTRL-^](#CTRL-^) to return to the
netrw browsing buffer.  Choose that by setting
this parameter to 0.
default: =0

### <a id="g:netrw_alto" class="section-title" href="#g:netrw_alto">Note:</a>
by setting this variable (see [netrw-o](#netrw-o))
default: =&sb           (see ['sb'](#'sb'))

### <a id="g:netrw_altv" class="section-title" href="#g:netrw_altv">Note:</a>
by setting this variable (see [netrw-v](#netrw-v))
default: =&spr          (see ['spr'](#'spr'))
...
Note that [g:netrw_preview](#g:netrw_preview) may be used
to get vertical splitting instead of
horizontal splitting.
=[servername,tab-number,window-number]
Given a [List](#List) such as this, a remote server
named by the "servername" will be used for
editing.  It will also use the specified tab
and window numbers to perform editing
(see [clientserver|, |netrw-ctrl-r](#clientserver|, |netrw-ctrl-r))
This option does not affect the production of
[:Lexplore](#:Lexplore) windows.

Related topics:
[g:netrw_alto|	|g:netrw_altv](#g:netrw_alto|	|g:netrw_altv)
[netrw-C|		|netrw-cr](#netrw-C|		|netrw-cr)
[netrw-ctrl-r](#netrw-ctrl-r)
...
extension.  (see [netrw_filehandler](#netrw_filehandler)).
...
remote URL.  (see [netrw-handler](#netrw-handler)).
...
browser's "x" command (see [netrw-x](#netrw-x)).  If
present, g:Netrw_corehandler specifies
either one or more function references
(see [Funcref](#Funcref)).  (the capital g:Netrw...
...
['cursorline'| (cul) and |'cursorcolumn'](#'cursorline'| (cul) and |'cursorcolumn')
...
Where
u-cul : user's ['cursorline'](#'cursorline')   initial setting used
u-cuc : user's ['cursorcolumn'](#'cursorcolumn') initial setting used
U-cul : user's ['cursorline'](#'cursorline')   current setting used
U-cuc : user's ['cursorcolumn'](#'cursorcolumn') current setting used
cul   : ['cursorline'](#'cursorline')   will be locally set
cuc   : ['cursorcolumn'](#'cursorcolumn') will be locally set

The "initial setting" means the values of
the ['cuc'| and |'cul'](#'cuc'| and |'cul') settings in effect when
netrw last saw [g:netrw_cursor](#g:netrw_cursor) >= 5 or when
...
history.
(related: [netrw-qb| |netrw-u| |netrw-U](#netrw-qb| |netrw-u| |netrw-U))
...
[g:netrw_maxfilenamelen](#g:netrw_maxfilenamelen), which affects
...
(or [netrw-ctrl-l](#netrw-ctrl-l) is used).
...
such a directory listing with [netrw-ctrl-l](#netrw-ctrl-l).
...
value is:
'^total\s\+\d\+$\|
### <a id="^Trying\s\+\d\+.$\[" class="section-title" href="#^Trying\s\+\d\+.$\](#" class="section-title" href="#^Trying\s\+\d\+.$\)">Note:</a>
...
This option controls how gx ([netrw-gx](#netrw-gx)) picks
up the text under the cursor.  See [expand()](#expand())
for possibilities.
...
.netrwhist).
Netrw uses [expand()](#expand()) on the string.
default: stdpath("data") (see [stdpath()](#stdpath()))
...
b:netrw_curdir (also see [netrw-cd](#netrw-cd))

### <a id="g:netrw_keepj" class="section-title" href="#g:netrw_keepj">Note:</a>
[:jumps](#:jumps) table unaffected.
=""      netrw will not use [:keepjumps](#:keepjumps) with
...
Patterns are regular expressions (see [regexp](#regexp))
There's some special support for git-ignore
files: you may add the output from the helper
function 'netrw_gitignore#Hide() automatically
hiding all gitignored files.
For more details see [netrw-gitignore](#netrw-gitignore).
...
Copies marked files ([netrw-mf](#netrw-mf)) to target
directory ([netrw-mt|, |netrw-mc](#netrw-mt|, |netrw-mc))

### <a id="g:netrw_localcopycmdopt	=''" class="section-title" href="#g:netrw_localcopycmdopt	=''">Note:</a>
=' \c copy'                     Windows
Options for the [g:netrw_localcopycmd](#g:netrw_localcopycmd)
...
Options for [g:netrw_localcopydircmd](#g:netrw_localcopydircmd)
...
=" /c mkdir"                    Windows
Options for [g:netrw_localmkdir](#g:netrw_localmkdir)

### <a id="g:netrw_localmovecmd	="mv"" class="section-title" href="#g:netrw_localmovecmd	="mv"">Note:</a>
=expand("$COMSPEC")             Windows
Moves marked files ([netrw-mf](#netrw-mf)) to target
directory ([netrw-mt|, |netrw-mm](#netrw-mt|, |netrw-mm))

### <a id="g:netrw_localmovecmdopt	=""" class="section-title" href="#g:netrw_localmovecmdopt	=""">Note:</a>
=" /c move"                     Windows
Options for [g:netrw_localmovecmd](#g:netrw_localmovecmd)
...
have patch#1107.  Otherwise, [delete()](#delete())
is used with the "d" option.

### <a id="g:netrw_localrmdiropt	=""" class="section-title" href="#g:netrw_localrmdiropt	=""">Note:</a>
=" /c rmdir"                    Windows
Options for [g:netrw_localrmdir](#g:netrw_localrmdir)
...
via ssh  (also see [g:netrw_remote_mkdir](#g:netrw_remote_mkdir))
...
="H" : human-readable (ex. 5K, 4M, 3G)
uses 1024 base
The long listing ([netrw-i](#netrw-i)) and query-file
maps ([netrw-qf](#netrw-qf)) will display file size
using the specified style.

### <a id="g:netrw_usetab" class="section-title" href="#g:netrw_usetab">Note:</a>
the <tab> map supporting shrinking/expanding a
Lexplore or netrw window will be enabled.
(see [netrw-c-tab](#netrw-c-tab))
...
via ftp  (also see [g:netrw_mkdir_cmd](#g:netrw_mkdir_cmd))
default: "mkdir"
...
You may also use the [:Rexplore](#:Rexplore) command to do
the same thing.
...
[netrw-ctrl-r](#netrw-ctrl-r) to use for its server.
default: "NETRWSERVER"
...
### <a id="g:netrw_sort_options	sorting is done using [:sort|; this" class="section-title" href="#g:netrw_sort_options	sorting is done using |:sort](#:sort|; this" class="section-title" href="#g:netrw_sort_options	sorting is done using |:sort); this">Note:</a>
...
['suffixes'](#'suffixes') are also added to the special
file highlighting group.
These syntax highlighting groups are linked
to netrwGray or Folded by default
(see [hl-Folded](#hl-Folded)), but one may put lines like
```
...
"o" (see [netrw-o|), "v" (see |netrw-v](#netrw-o|), "v" (see |netrw-v)),
...
split will be made (ie. ['equalalways'](#'equalalways') will
...
between 1 and ['tabstop'](#'tabstop'), wide CJK as 2
...
(also see: [netrw-browser-var| |netrw-protocol| |netrw-variables](#netrw-browser-var| |netrw-protocol| |netrw-variables))
...
unless [g:netrw_keepdir](#g:netrw_keepdir) is set to 0 in the user's <.vimrc>.  One may select
a file using the local browser (by putting the cursor on it) and pressing
"O" will then "obtain" the file; ie. copy it to Vim's current directory.

Related topics:
* To see what the current directory is, use [:pwd](#:pwd)
* To make the currently browsed directory the current directory, see
[netrw-cd](#netrw-cd)
* To automatically make the currently browsed directory the current
directory, see [g:netrw_keepdir](#g:netrw_keepdir).
...
If Lexplore ([netrw-:Lexplore](#netrw-:Lexplore)) is in use, the new file will be generated
in the [g:netrw_chgwin](#g:netrw_chgwin) window.
...
the browser (where the cursor will remain) and the file (see [:pedit](#:pedit)).  By
default, the split will be taken horizontally; one may use vertical splitting
if one has set [g:netrw_preview](#g:netrw_preview) first.
...
([netrw-:Lexplore|) or |g:netrw_chgwin](#netrw-:Lexplore|) or |g:netrw_chgwin) of interest

Also see: [g:netrw_chgwin| |netrw-P| |'previewwindow'| |CTRL-W_z| |:pclose](#g:netrw_chgwin| |netrw-P| |'previewwindow'| |CTRL-W_z| |:pclose)
...
accessed) window (see :he [CTRL-W_p](#CTRL-W_p)), press a "P".  If there's only one
...
Related Actions [netrw-cr| |netrw-o| |netrw-t| |netrw-v](#netrw-cr| |netrw-o| |netrw-t| |netrw-v)
Associated setting variables:
[g:netrw_alto](#g:netrw_alto)    control above/below splitting
[g:netrw_altv](#g:netrw_altv)    control right/left splitting
[g:netrw_preview](#g:netrw_preview) control horizontal vs vertical splitting
[g:netrw_winsize](#g:netrw_winsize) control initial sizing

Also see: [g:netrw_chgwin| |netrw-p](#g:netrw_chgwin| |netrw-p)
...
Associated setting variable: [g:netrw_sort_direction](#g:netrw_sort_direction)


RENAMING FILES OR DIRECTORIES	*netrw-move* *netrw-rename* *netrw-R* {{{2

If there are no marked files: (see [netrw-mf](#netrw-mf))
...
If there are marked files:  (see [netrw-mf](#netrw-mf))
...
V ([linewise-visual](#linewise-visual)) when using thin style.

See [cmdline-editing](#cmdline-editing) for more on how to edit the command line; in particular,
...
Related topics:               [netrw-r| |netrw-S](#netrw-r| |netrw-S)
Associated setting variables: [g:netrw_sort_by| |g:netrw_sort_sequence](#g:netrw_sort_by| |g:netrw_sort_sequence)
...
:NetrwC [win#] command, or by setting [g:netrw_chgwin](#g:netrw_chgwin) to the selected window
number.  Subsequent selection of a file to edit ([netrw-cr](#netrw-cr)) will use that
window.

* C : by itself, will select the current window holding a netrw buffer
for subsequent editing via [netrw-cr](#netrw-cr).  The C mapping is only available
while in netrw buffers.

* [count]C : the count will be used as the window number to be used
for subsequent editing via [netrw-cr](#netrw-cr).

* :NetrwC will set [g:netrw_chgwin](#g:netrw_chgwin) to the current window

* :NetrwC win#  will set [g:netrw_chgwin](#g:netrw_chgwin) to the specified window
number
...
Related topics:			[netrw-cr| |g:netrw_browse_split](#netrw-cr| |g:netrw_browse_split)
Associated setting variables:	[g:netrw_chgwin](#g:netrw_chgwin)


SHRINKING OR EXPANDING A NETRW OR LEXPLORE WINDOW	*netrw-c-tab* {{{2

The <c-tab> key will toggle a netrw or [:Lexplore](#:Lexplore) window's width,
but only if [g:netrw_usetab](#g:netrw_usetab) exists and is non-zero (and, of course,
...
(between [g:netrw_wiw](#g:netrw_wiw) and its original width)

### <a id=" Else if there is a [:Lexplore| window in the current tab, toggle" class="section-title" href="# Else if there is a |:Lexplore](#:Lexplore| window in the current tab, toggle" class="section-title" href="# Else if there is a |:Lexplore) window in the current tab, toggle">Note:</a>
its width

### <a id=" Else bring up a [:Lexplore| window" class="section-title" href="# Else bring up a |:Lexplore](#:Lexplore| window" class="section-title" href="# Else bring up a |:Lexplore) window">Note:</a>

If [g:netrw_usetab](#g:netrw_usetab) exists and is zero, or if there is a pre-existing mapping
for <c-tab>, then the <c-tab> will not be mapped.  One may map something other
than a <c-tab>, too: (but you'll still need to have had [g:netrw_usetab](#g:netrw_usetab) set).
```
...
Related topics:			[:Lexplore](#:Lexplore)
Associated setting variable:	[g:netrw_usetab](#g:netrw_usetab)


### <a id="netrw-usermaps {{{1" class="section-title" href="#netrw-usermaps {{{1">User Specified Maps</a>

One may make customized user maps.  Specify a variable, [g:Netrw_UserMaps](#g:Netrw_UserMaps),
to hold a [List](#List) of lists of keymap strings and function names:
```
...
When netrw is setting up maps for a netrw buffer, if [g:Netrw_UserMaps](#g:Netrw_UserMaps)
exists, then the internal function netrw#UserMaps(islocal) is called.
This function goes through all the entries in the [g:Netrw_UserMaps](#g:Netrw_UserMaps) list:
...
Put the following statement into your [vimrc](#vimrc):
```
...
Put the following line in your [vimrc](#vimrc):
```
...
The first one ([g:netrw_ssh_cmd](#g:netrw_ssh_cmd)) is the most important; most
...
appear.  Please try setting [g:netrw_use_noswf](#g:netrw_use_noswf) to 0
in your <.vimrc>:
```
let g:netrw_use_noswf= 0
...
causes the "Press ENTER..." prompt.  So:  read [hit-enter](#hit-enter);
I also suggest that you set your ['cmdheight'](#'cmdheight') to 2 (or more) in
your <.vimrc> file.
...
wish the [:Lexplore|r window to start with (see |g:netrw_winsize](#:Lexplore|r window to start with (see |g:netrw_winsize)
...
### <a id=" Resize the windows as you wish (see [CTRL-W_<| and" class="section-title" href="# Resize the windows as you wish (see |CTRL-W_<](#CTRL-W_<| and" class="section-title" href="# Resize the windows as you wish (see |CTRL-W_<) and">Note:</a>
[CTRL-W_>](#CTRL-W_>)).  If you're using gvim, you can drag
...
netrw browser, select a file, then press P  (see [CTRL-W_h](#CTRL-W_h)
and [netrw-P](#netrw-P)).  If you're using gvim, you can press
...
You may need to change [g:netrw_sepchr| and/or |g:netrw_xstrlen](#g:netrw_sepchr| and/or |g:netrw_xstrlen).
...
(drchip) I am able to duplicate this problem with ['acd'](#'acd') set.
...
With [g:netrw_bufsettings](#g:netrw_bufsettings), you can control netrw's buffer
...
### <a id="\ (v:servername =~ 'GVIM\d' [| v:servername == "")" class="section-title" href="#\ (v:servername =~ 'GVIM\d' |](#| v:servername == "")" class="section-title" href="#\ (v:servername =~ 'GVIM\d' |) v:servername == "")">Note:</a>
\ [ e . ](# e . ) endif
augroup END
...
[g:netrw_localrmdir|, |g:netrw_mkdir_cmd](#g:netrw_localrmdir|, |g:netrw_mkdir_cmd)).
...
shown by netrw (see [g:netrw_keepdir](#g:netrw_keepdir)).
...
(see [:version](#:version))  (remove the embedded NOSPAM first)
```
...
to allow ['bh'](#'bh') to be set to delete when
rather than hide when g:netrw_fastbrowse
was zero.
### <a id=" Installed [g:netrw_clipboard| setting" class="section-title" href="# Installed |g:netrw_clipboard](#g:netrw_clipboard| setting" class="section-title" href="# Installed |g:netrw_clipboard) setting">Note:</a>
### <a id=" Installed option bypass for ['guioptions'|" class="section-title" href="# Installed option bypass for |'guioptions'](#'guioptions'|" class="section-title" href="# Installed option bypass for |'guioptions')">Note:</a>
a/A settings
...
but will emit an [E37](#E37) (although one cannot
add an ! to override)
...
buffers created by :Lexplore to ['bh'](#'bh')=wipe.
v169:	Dec 20, 2019	* (reported by amkarthik) that netrw's x
([netrw-x](#netrw-x)) would throw an error when
...
with additional [g:netrw_special_syntax](#g:netrw_special_syntax)
items
...
x and gx ([netrw-x| and |netrw-gx](#netrw-x| and |netrw-gx)) were no
...
settings: [g:netrw_localcopycmdopt](#g:netrw_localcopycmdopt)
[g:netrw_localcopydircmdopt| |g:netrw_localmkdiropt](#g:netrw_localcopydircmdopt| |g:netrw_localmkdiropt)
[g:netrw_localmovecmdopt| |g:netrw_localrmdiropt](#g:netrw_localmovecmdopt| |g:netrw_localrmdiropt)
...
### <a id=" Also fixed obtain ([netrw-O|) to be able to" class="section-title" href="# Also fixed obtain (|netrw-O](#netrw-O|) to be able to" class="section-title" href="# Also fixed obtain (|netrw-O)) to be able to">Note:</a>
obtain files with spaces in their names
### <a id="Dec 20, 2016	 (xc1427) Reported that using "I" ([netrw-I|)" class="section-title" href="#Dec 20, 2016	 (xc1427) Reported that using "I" (|netrw-I](#netrw-I|)" class="section-title" href="#Dec 20, 2016	 (xc1427) Reported that using "I" (|netrw-I))">Note:</a>
...
use [:edit| instead of |:read](#:edit| instead of |:read).  I also
...
(see [netrw-cb|, |netrw-cB|, and |netrw-cd](#netrw-cb|, |netrw-cB|, and |netrw-cd))
...
### <a id="Oct 17, 2017	 (Adam Faryna) reported that gn ([netrw-gn|)" class="section-title" href="#Oct 17, 2017	 (Adam Faryna) reported that gn (|netrw-gn](#netrw-gn|)" class="section-title" href="#Oct 17, 2017	 (Adam Faryna) reported that gn (|netrw-gn))">Note:</a>
...
refreshes.  However, inside a [:map-<expr>](#:map-<expr>),
tab and window changes are disallowed.  Fixed.
(affects netrw's s:LocalBrowseRefresh())
### <a id=" [g:netrw_localrmdir| not used any more, but" class="section-title" href="# |g:netrw_localrmdir](#g:netrw_localrmdir| not used any more, but" class="section-title" href="# |g:netrw_localrmdir) not used any more, but">Note:</a>
the relevant patch that causes [delete()](#delete()) to
take over was #1107 (not #1109).
### <a id=" [expand()| is now used on |g:netrw_home|;" class="section-title" href="# |expand()| is now used on |g:netrw_home](#expand()| is now used on |g:netrw_home|;" class="section-title" href="# |expand()| is now used on |g:netrw_home);">Note:</a>
...
See [g:netrw_suppress_gx_mesg](#g:netrw_suppress_gx_mesg)
...
banner
### <a id="Nov 16, 2015	 Added a <Plug>NetrwTreeSqueeze ([netrw-s-cr|)" class="section-title" href="#Nov 16, 2015	 Added a <Plug>NetrwTreeSqueeze (|netrw-s-cr](#netrw-s-cr|)" class="section-title" href="#Nov 16, 2015	 Added a <Plug>NetrwTreeSqueeze (|netrw-s-cr))">Note:</a>
...
retained?
### <a id="Nov 20, 2015	 Added [netrw-ma| and |netrw-mA| support" class="section-title" href="#Nov 20, 2015	 Added |netrw-ma| and |netrw-mA](#netrw-ma| and |netrw-mA| support" class="section-title" href="#Nov 20, 2015	 Added |netrw-ma| and |netrw-mA) support">Note:</a>
### <a id="Nov 20, 2015	 gx ([netrw-gx|) on a URL downloaded the" class="section-title" href="#Nov 20, 2015	 gx (|netrw-gx](#netrw-gx|) on a URL downloaded the" class="section-title" href="#Nov 20, 2015	 gx (|netrw-gx)) on a URL downloaded the">Note:</a>
file in addition to simply bringing up the
URL in a browser.  Fixed.
### <a id="Nov 23, 2015	 Added [g:netrw_sizestyle| support" class="section-title" href="#Nov 23, 2015	 Added |g:netrw_sizestyle](#g:netrw_sizestyle| support" class="section-title" href="#Nov 23, 2015	 Added |g:netrw_sizestyle) support">Note:</a>
...
maps.
### <a id="Jan 05, 2016	 [netrw-qL| implemented to mark files based" class="section-title" href="#Jan 05, 2016	 |netrw-qL](#netrw-qL| implemented to mark files based" class="section-title" href="#Jan 05, 2016	 |netrw-qL) implemented to mark files based">Note:</a>
upon [location-list|s; similar to |netrw-qF](#location-list|s; similar to |netrw-qF).
...
### <a id="Jan 28, 2016	 changed to using [winsaveview()| and" class="section-title" href="#Jan 28, 2016	 changed to using |winsaveview()](#winsaveview()| and" class="section-title" href="#Jan 28, 2016	 changed to using |winsaveview()) and">Note:</a>
...
delay.
### <a id="Jun 26, 2015	 [netrw-gn| mapping implemted" class="section-title" href="#Jun 26, 2015	 |netrw-gn](#netrw-gn| mapping implemted" class="section-title" href="#Jun 26, 2015	 |netrw-gn) mapping implemted">Note:</a>
...
### <a id="Jul 08, 2015	 Several changes to keep the [:jumps| table" class="section-title" href="#Jul 08, 2015	 Several changes to keep the |:jumps](#:jumps| table" class="section-title" href="#Jul 08, 2015	 Several changes to keep the |:jumps) table">Note:</a>
correct when working with
[g:netrw_fastbrowse](#g:netrw_fastbrowse) set to 2
...
(using %-S instead of %-s with a [printf()](#printf())
...
(see [netrw-c-tab](#netrw-c-tab))
v153:	May 13, 2014	* added another [g:netrw_ffkeep](#g:netrw_ffkeep) usage {{{2
...
### <a id="May 16, 2014	 introduced the [netrw-ctrl-r| functionality" class="section-title" href="#May 16, 2014	 introduced the |netrw-ctrl-r](#netrw-ctrl-r| functionality" class="section-title" href="#May 16, 2014	 introduced the |netrw-ctrl-r) functionality">Note:</a>
### <a id="May 17, 2014	 introduced the [netrw-:NetrwMB| functionality" class="section-title" href="#May 17, 2014	 introduced the |netrw-:NetrwMB](#netrw-:NetrwMB| functionality" class="section-title" href="#May 17, 2014	 introduced the |netrw-:NetrwMB) functionality">Note:</a>
### <a id=" mb and mB ([netrw-mb|, |netrw-mB|) will" class="section-title" href="# mb and mB (|netrw-mb|, |netrw-mB](#netrw-mb|, |netrw-mB|) will" class="section-title" href="# mb and mB (|netrw-mb|, |netrw-mB)) will">Note:</a>
...
the [:Lexplore](#:Lexplore) window 100% high, optionally
...
### <a id="Jun 10, 2014	 [g:netrw_gx| introduced to allow users to" class="section-title" href="#Jun 10, 2014	 |g:netrw_gx](#g:netrw_gx| introduced to allow users to" class="section-title" href="#Jun 10, 2014	 |g:netrw_gx) introduced to allow users to">Note:</a>
override default "<cfile>" with the gx
([netrw-gx](#netrw-gx)) map
### <a id="Jun 11, 2014	 gx ([netrw-gx|), with |'autowrite'| set," class="section-title" href="#Jun 11, 2014	 gx (|netrw-gx|), with |'autowrite'](#netrw-gx|), with |'autowrite'| set," class="section-title" href="#Jun 11, 2014	 gx (|netrw-gx|), with |'autowrite') set,">Note:</a>
will write modified files.  s:NetrwBrowseX()
will now save, turn off, and restore the
['autowrite'](#'autowrite') setting.
...
### <a id="Jul 05, 2014	 [netrw-mv| and |netrw-mX| commands included" class="section-title" href="#Jul 05, 2014	 |netrw-mv| and |netrw-mX](#netrw-mv| and |netrw-mX| commands included" class="section-title" href="#Jul 05, 2014	 |netrw-mv| and |netrw-mX) commands included">Note:</a>
### <a id="Jul 09, 2014	 [g:netrw_keepj| included, allowing optional" class="section-title" href="#Jul 09, 2014	 |g:netrw_keepj](#g:netrw_keepj| included, allowing optional" class="section-title" href="#Jul 09, 2014	 |g:netrw_keepj) included, allowing optional">Note:</a>
keepj
...
### <a id="Oct 07, 2014	 gx (see [netrw-gx|) when atop a directory," class="section-title" href="#Oct 07, 2014	 gx (see |netrw-gx](#netrw-gx|) when atop a directory," class="section-title" href="#Oct 07, 2014	 gx (see |netrw-gx)) when atop a directory,">Note:</a>
will now do [gf](#gf) instead
...
[g:netrw_file_cmd](#g:netrw_file_cmd) (typically elinks or links)
### <a id="Dec 02, 2014	 began work on having mc ([netrw-mc|) copy" class="section-title" href="#Dec 02, 2014	 began work on having mc (|netrw-mc](#netrw-mc|) copy" class="section-title" href="#Dec 02, 2014	 began work on having mc (|netrw-mc)) copy">Note:</a>
...
thin and tree modes
### <a id="Dec 30, 2014	 (issue#231) [:ls| was not showing" class="section-title" href="#Dec 30, 2014	 (issue#231) |:ls](#:ls| was not showing" class="section-title" href="#Dec 30, 2014	 (issue#231) |:ls) was not showing">Note:</a>
remote-file buffers reliably.  Fixed.
v152:	Apr 08, 2014	* uses the ['noswapfile'](#'noswapfile') option (requires {{{2
vim 7.4 with patch 213)
### <a id=" (Enno Nagel) turn ['rnu'| off in netrw" class="section-title" href="# (Enno Nagel) turn |'rnu'](#'rnu'| off in netrw" class="section-title" href="# (Enno Nagel) turn |'rnu') off in netrw">Note:</a>
buffers.
...
the [netrw-O](#netrw-O) command can obtain remote
files via sftp.
### <a id=" added [count]C support (see [netrw-C|)" class="section-title" href="# added [count]C support (see |netrw-C](#netrw-C|)" class="section-title" href="# added [count]C support (see |netrw-C))">Note:</a>
### <a id="Apr 18, 2014	 when [g:netrw_chgwin| is one more than" class="section-title" href="#Apr 18, 2014	 when |g:netrw_chgwin](#g:netrw_chgwin| is one more than" class="section-title" href="#Apr 18, 2014	 when |g:netrw_chgwin) is one more than">Note:</a>
...
handling.
### <a id=" [:Lexplore| path: will be used to update" class="section-title" href="# |:Lexplore](#:Lexplore| path: will be used to update" class="section-title" href="# |:Lexplore) path: will be used to update">Note:</a>
a left-side netrw browsing directory.
### <a id="Mar 12, 2014	 [netrw-s-cr|: use <s-cr>" class="section-title" href="#Mar 12, 2014	 |netrw-s-cr](#netrw-s-cr|: use <s-cr>" class="section-title" href="#Mar 12, 2014	 |netrw-s-cr): use <s-cr>">Note:</a>
...
gzip.vim).  Fixed.
### <a id="Mar 14, 2014	 Implemented :MF and :MT (see [netrw-:MF|" class="section-title" href="#Mar 14, 2014	 Implemented :MF and :MT (see |netrw-:MF](#netrw-:MF|" class="section-title" href="#Mar 14, 2014	 Implemented :MF and :MT (see |netrw-:MF)">Note:</a>
and [netrw-:MT](#netrw-:MT), respectively)
### <a id="Mar 17, 2014	 [:Ntree| [dir] wasn't working properly; fixed" class="section-title" href="#Mar 17, 2014	 |:Ntree](#:Ntree| [dir] wasn't working properly; fixed" class="section-title" href="#Mar 17, 2014	 |:Ntree) [dir] wasn't working properly; fixed">Note:</a>
...
netrw buffers will remain as ['bt'](#'bt')=nofile.
...
### <a id=" Fixed [netrw-d| command when applied with ftp" class="section-title" href="# Fixed |netrw-d](#netrw-d| command when applied with ftp" class="section-title" href="# Fixed |netrw-d) command when applied with ftp">Note:</a>
...
### <a id="Aug 27, 2013	 introduced [g:netrw_altfile| option" class="section-title" href="#Aug 27, 2013	 introduced |g:netrw_altfile](#g:netrw_altfile| option" class="section-title" href="#Aug 27, 2013	 introduced |g:netrw_altfile) option">Note:</a>
### <a id="Sep 05, 2013	 s:Strlen() now uses [strdisplaywidth()|" class="section-title" href="#Sep 05, 2013	 s:Strlen() now uses |strdisplaywidth()](#strdisplaywidth()|" class="section-title" href="#Sep 05, 2013	 s:Strlen() now uses |strdisplaywidth())">Note:</a>
...
command line.
### <a id="Nov 09, 2013	 [:Lexplore| introduced" class="section-title" href="#Nov 09, 2013	 |:Lexplore](#:Lexplore| introduced" class="section-title" href="#Nov 09, 2013	 |:Lexplore) introduced">Note:</a>
...
### <a id=" New command installed: [:Ntree|" class="section-title" href="# New command installed: |:Ntree](#:Ntree|" class="section-title" href="# New command installed: |:Ntree)">Note:</a>
...
[netrw-P](#netrw-P).  Fixed.
### <a id="Jan 06, 2014	 Fixed a problem with [netrw-P| when the" class="section-title" href="#Jan 06, 2014	 Fixed a problem with |netrw-P](#netrw-P| when the" class="section-title" href="#Jan 06, 2014	 Fixed a problem with |netrw-P) when the">Note:</a>
...
### <a id="May 02, 2013	 introduced [g:netrw_bannerbackslash| as" class="section-title" href="#May 02, 2013	 introduced |g:netrw_bannerbackslash](#g:netrw_bannerbackslash| as" class="section-title" href="#May 02, 2013	 introduced |g:netrw_bannerbackslash) as">Note:</a>
...
07/29/09 : banner	:[g:netrw_banner](#g:netrw_banner) can be used to suppress the

...
rpcnotify({channel}, {event} [, {args}...])
				Sends an [RPC](undefined#RPC) notification to {channel}
...
### <a id="floor()" class="section-title" href="#floor()">floor({expr})</a>
		...
		{expr} as a [Float](undefined#Float) (round down).
		{expr} must evaluate to a [Float](undefined#Float) or a [Number](undefined#Number).
		Returns 0.0 if {expr} is not a [Float](undefined#Float) or a [Number](undefined#Number).
		Examples:
			echo floor(1.856)
...
### <a id="fmod()" class="section-title" href="#fmod()">fmod({expr1}, {expr2})</a>
		...
		returned is zero.  The value returned is a [Float](undefined#Float).
		{expr1} and {expr2} must evaluate to a [Float](undefined#Float) or a [Number](undefined#Number).
		Returns 0.0 if {expr1} or {expr2} is not a [Float](undefined#Float) or a
		[Number](undefined#Number).
		Examples:
...
### <a id="fnamemodify()" class="section-title" href="#fnamemodify()">fnamemodify({fname}, {mods})</a>
		...
		command line.  See [filename-modifiers](undefined#filename-modifiers).
		Example:
...
function({name} [, {arglist}] [, {dict}])
		Return a [Funcref](undefined#Funcref) variable that refers to function {name}.
...
```
		...
		arguments, but after any argument from [method](undefined#method).  Example:
```			func Callback(arg1, arg2, name)
		...
		With a [method](undefined#method):
...
### <a id="garbagecollect()" class="section-title" href="#garbagecollect()">garbagecollect([{atexit}])</a>
		Cleanup unused [Lists](undefined#Lists) and [Dictionaries](undefined#Dictionaries) that have circular
		references.
		...
		This is useful if you have deleted a very big [List](undefined#List) and/or
		[Dictionary](undefined#Dictionary) with circular references in a script that runs
		for a long time.
...
### <a id="get()" class="section-title" href="#get()">get({list}, {idx} [, {default}])</a>
		Get item {idx} from [List](undefined#List) {list}.  When this item is not
		available return {default}.  Return zero when {default} is
		omitted.
		Can also be used as a [method](undefined#method):
			mylist->get(idx)
get({blob}, {idx} [, {default}])
		Get byte {idx} from [Blob](undefined#Blob) {blob}.  When this byte is not
		available return {default}.  Return -1 when {default} is
		omitted.
get({dict}, {key} [, {default}])
		Get item with key {key} from [Dictionary](undefined#Dictionary) {dict}.  When this
...
getbufinfo([{dict}])
		...
		When the argument is a [Dictionary](undefined#Dictionary) only the buffers matching
...
```
			...
			windows		List of [window-ID](undefined#window-ID)s that display this
					buffer
...
getbufline({buf}, {lnum} [, {end}])
		Return a [List](undefined#List) with the lines starting from {lnum} to {end}
		(inclusive) in the buffer {buf}.  If {end} is omitted, a
		[List](undefined#List) with only the line {lnum} is returned.
		...
		lines in the buffer, an empty [List](undefined#List) is returned.

		When {end} is greater than the number of lines in the buffer,
		it is treated as {end} is set to the number of lines in the
		buffer.  When {end} is before {lnum} an empty [List](undefined#List) is
		returned.

		This function works only for loaded buffers.  For unloaded and
		non-existing buffers, an empty [List](undefined#List) is returned.
...
### <a id="getbufvar()" class="section-title" href="#getbufvar()">getbufvar({buf}, {varname} [, {def}])</a>
		...
		When {varname} is empty returns a [Dictionary](undefined#Dictionary) with all the
		buffer-local variables.
		When {varname} is equal to "&" returns a [Dictionary](undefined#Dictionary) with all
...
### <a id="getchangelist()" class="section-title" href="#getchangelist()">getchangelist([{buf}])</a>
		Returns the [changelist](undefined#changelist) for the buffer {buf}. For the use
...
### <a id="getcharsearch()" class="section-title" href="#getcharsearch()">getcharsearch()</a>
		...
		with the following entries:
		    ...
		    char	character previously used for a character
				search ([t](undefined#t), [f](undefined#f), [T](undefined#T), or [F](undefined#F)); empty string
				if no character search has been performed
		    forward	direction of character search; 1 for forward,
				0 for backward
		    until	type of character search; 1 for a [t](undefined#t) or [T](undefined#T)
				character search, 0 for an [f](undefined#f) or [F](undefined#F)
				character search
...
### <a id="getcmdwintype()" class="section-title" href="#getcmdwintype()">getcmdwintype()</a>
		Return the current [command-line-window](/neovim-docs-web/en/vim/cmdline#command-line-window) type. Possible return
...
### <a id="getcompletion()" class="section-title" href="#getcompletion()">getcompletion({pat}, {type} [, {filtered}])</a>
		...
		cmdline		[cmdline-completion](/neovim-docs-web/en/vim/cmdline#cmdline-completion) result
		color		color schemes
		command		Ex command
		compiler	compilers
		...
		option		options
		packadd		optional package [pack-add](undefined#pack-add) names
		...
		See [wildcards](undefined#wildcards) for the use of special characters in {pat}.

		If the optional {filtered} flag is set to 1, then 'wildignore'
		is applied to filter the results.  Otherwise all the matches
		are returned. The 'wildignorecase' option always applies.

		If {type} is "cmdline", then the [cmdline-completion](/neovim-docs-web/en/vim/cmdline#cmdline-completion) result is
...
getcurpos([{winid}])
		...
		be the window number or the [window-ID](undefined#window-ID).  The last known
...
### <a id="getcwd()" class="section-title" href="#getcwd()">getcwd([{winnr} [, {tabnr}]])</a>
		...
		[current-directory](/neovim-docs-web/en/vim/editing#current-directory). With {winnr} or {tabnr} the working
		...
		Throw error if the arguments are invalid. [E5000](undefined#E5000) [E5001](undefined#E5001) [E5002](undefined#E5002)
...
### <a id="getenv()" class="section-title" href="#getenv()">getenv({name})</a>
		...
		See also [expr-env](undefined#expr-env).
...
### <a id="getfontname()" class="section-title" href="#getfontname()">getfontname([{name}])</a>
		...
		[hl-Normal](undefined#hl-Normal).
		With an argument a check is done whether String {name} is a
		valid font name.  If not then an empty string is returned.
		Otherwise the actual font name is returned, or {name} if the
		GUI does not support obtaining the real name.
		Only works when the GUI is running, thus not in your vimrc or
		gvimrc file.  Use the [GUIEnter](undefined#GUIEnter) autocommand to use this
...
### <a id="getjumplist()" class="section-title" href="#getjumplist()">getjumplist([{winnr} [, {tabnr}]])</a>
		Returns the [jumplist](undefined#jumplist) for the specified window.

		Without arguments use the current window.
		With {winnr} only use this window in the current tab page.
		{winnr} can also be a [window-ID](undefined#window-ID).
		With {winnr} and {tabnr} use the window in the specified tab
...
getline({lnum} [, {end}])
		...
		When {end} is given the result is a [List](undefined#List) where each item is
		a line from the current buffer in the range {lnum} to {end},
		including line {end}.
		{end} is used in the same way as {lnum}.
		Non-existing lines are silently omitted.
		When {end} is before {lnum} an empty [List](undefined#List) is returned.
		Example:
			:let start = line('.')
			:let end = search("^$") - 1
...
### <a id="getloclist()" class="section-title" href="#getloclist()">getloclist({nr} [, {what}])</a>
		Returns a [List](undefined#List) with all the entries in the location list for
		window {nr}.  {nr} can be the window number or the [window-ID](undefined#window-ID).
		...
		the following item is supported by |getloclist()|:
			...
			filewinid	id of the window used to display files
					...
					[location-list-file-window](undefined#location-list-file-window) for more
					details.

		Returns a [Dictionary](undefined#Dictionary) with default values if there is no
		location list for the window {nr}.
		Returns an empty Dictionary if window {nr} does not exist.

		Examples (See also [getqflist-examples](undefined#getqflist-examples)):
			:echo getloclist(3, {'all': 0})
			:echo getloclist(5, {'filewinid': 0})


### <a id="getmarklist()" class="section-title" href="#getmarklist()">getmarklist([{buf}])</a>
		Without the {buf} argument returns a [List](undefined#List) with information
		about all the global marks. [mark](undefined#mark)
		...
		Each item in the returned List is a [Dict](undefined#Dict) with the following:
		    mark   name of the mark prefixed by "'"
		    pos	   a [List](undefined#List) with the position of the mark:
				[bufnum, lnum, col, off]
...
### <a id="getmatches()" class="section-title" href="#getmatches()">getmatches([{win}])</a>
		Returns a [List](undefined#List) with all matches previously defined for the
...
getpos({expr})	Get the position for String {expr}.  For possible values of
		...
		|getcurpos()|.
		The result is a [List](undefined#List) with four numbers:
		    [bufnum, lnum, col, off]
...
### <a id="getqflist()" class="section-title" href="#getqflist()">getqflist([{what}])</a>
		Returns a [List](undefined#List) with all the current quickfix errors.  Each
		list item is a dictionary with these entries:
			...
			vcol	[TRUE](undefined#TRUE): "col" is visual column
				[FALSE](undefined#FALSE): "col" is byte index
			nr	error number
			pattern	search pattern used to locate the error
			text	description of the error
			type	type of the error, 'E', '1', etc.
			valid	[TRUE](undefined#TRUE): recognized error message
...
```
		...
		following string items are supported in {what}:
			changedtick	get the total number of changes made
					to the list [quickfix-changedtick](undefined#quickfix-changedtick)
			context	get the [quickfix-context](undefined#quickfix-context)
			efm	errorformat to use when parsing "lines". If
				not present, then the 'errorformat' option
				value is used.
			id	get information for the quickfix list with
				[quickfix-ID](undefined#quickfix-ID); zero means the id for the
				current list or the list specified by "nr"
			idx	get information for the quickfix entry at this
				index in the list specified by "id" or "nr".
				If set to zero, then uses the current entry.
				See [quickfix-index](undefined#quickfix-index)
			items	quickfix list entries
			lines	parse a list of lines using 'efm' and return
				the resulting entries.  Only a [List](undefined#List) type is
				accepted.  The current quickfix list is not
				modified. See [quickfix-parse](undefined#quickfix-parse).
			nr	get information for this quickfix list; zero
				means the current quickfix list and "$" means
				the last quickfix list
			qfbufnr number of the buffer displayed in the quickfix
				window. Returns 0 if the quickfix buffer is
				not present. See [quickfix-buffer](undefined#quickfix-buffer).
			size	number of entries in the quickfix list
			title	get the list title [quickfix-title](undefined#quickfix-title)
			winid	get the quickfix [window-ID](undefined#window-ID)
		...
		The returned dictionary contains the following entries:
			changedtick	total number of changes made to the
					list [quickfix-changedtick](undefined#quickfix-changedtick)
			context	quickfix list context. See [quickfix-context](undefined#quickfix-context)
				If not present, set to "".
			id	quickfix list ID [quickfix-ID](undefined#quickfix-ID). If not
				present, set to 0.
			...
			title	quickfix list title text. If not present, set
				to "".
			winid	quickfix [window-ID](undefined#window-ID). If not present, set to 0
...
#### <a id=":let cliptext = getreg('')" class="section-title" href="#:let cliptext = getreg('')">```</a>
		...
		If {list} is present and [TRUE](undefined#TRUE), the result type is changed
		to [List](undefined#List). Each list item is one text line. Use it if you care
		about zero bytes possibly present inside register: without
		third argument both NLs and zero bytes are represented as NLs
		(see [NL-used-for-Nul](undefined#NL-used-for-Nul)).
...
### <a id="getreginfo()" class="section-title" href="#getreginfo()">getreginfo([{regname}])</a>
		...
		Dictionary with the following entries:
			...
			points_to	for the unnamed register, gives the
					...
					currently pointed to (see [quotequote](undefined#quotequote)).
...
### <a id="getregtype()" class="section-title" href="#getregtype()">getregtype([{regname}])</a>
		...
		The value will be one of:
		    "v"			for [charwise](undefined#charwise) text
		    "V"			for [linewise](undefined#linewise) text
		    "<CTRL-V>{width}"	for [blockwise-visual](undefined#blockwise-visual) text
...
### <a id="gettabinfo()" class="section-title" href="#gettabinfo()">gettabinfo([{tabnr}])</a>
		...
		tab pages is returned as a [List](undefined#List). Each List item is a
		[Dictionary](undefined#Dictionary).  Otherwise, {tabnr} specifies the tab page
		number and information about that one is returned.  If the tab
		page does not exist an empty List is returned.

		Each List item is a [Dictionary](undefined#Dictionary) with the following entries:
			tabnr		tab page number.
			variables	a reference to the dictionary with
					tabpage-local variables
			windows		List of [window-ID](undefined#window-ID)s in the tab page.
...
### <a id="gettabwinvar()" class="section-title" href="#gettabwinvar()">gettabwinvar({tabnr}, {winnr}, {varname} [, {def}])</a>
		...
		window-local options in a [Dictionary](undefined#Dictionary).
...
### <a id="gettagstack()" class="section-title" href="#gettagstack()">gettagstack([{winnr}])</a>
		...
		See [tagstack](undefined#tagstack) for more information about the tag stack.

		Can also be used as a [method](undefined#method):
			GetWinnr()->gettagstack()

### <a id="getwininfo()" class="section-title" href="#getwininfo()">getwininfo([{winid}])</a>
		Returns information about windows as a [List](undefined#List) with Dictionaries.

		If {winid} is given Information about the window with that ID
		is returned, as a [List](undefined#List) with one item.  If the window does not
		...
		Each List item is a [Dictionary](undefined#Dictionary) with the following entries:
			...
			winid		[window-ID](undefined#window-ID)
			winnr		window number
...
### <a id="getwinpos()" class="section-title" href="#getwinpos()">getwinpos([{timeout}])</a>
		The result is a [List](undefined#List) with two numbers, the result of
...
### <a id="glob()" class="section-title" href="#glob()">glob({expr} [, {nosuf} [, {list} [, {alllinks}]]])</a>
		Expand the file wildcards in {expr}.  See [wildcards](undefined#wildcards) for the
		use of special characters.

		Unless the optional {nosuf} argument is given and is [TRUE](undefined#TRUE),
		the 'suffixes' and 'wildignore' options apply: Names matching
		one of the patterns in 'wildignore' will be skipped and
		'suffixes' affect the ordering of matches.
		'wildignorecase' always applies.

		When {list} is present and it is [TRUE](undefined#TRUE) the result is a [List](undefined#List)
		...
		[TRUE](undefined#TRUE) then all symbolic links are included.
...
```
		...
		features. [feature-compile](undefined#feature-compile)
...
### <a id="feature-list" class="section-title" href="#feature-list">Note:</a>
			...
			acl		[ACL](undefined#ACL) support.
			bsd		BSD system (not macOS, use "mac" for that).
			clipboard	[clipboard](undefined#clipboard) provider is available.
			...
			mac		MacOS system.
			nvim		This is Nvim.
			python3		Legacy Vim [python3](/neovim-docs-web/en/neovim/if_pyth#python3) interface. [has-python](undefined#has-python)
			pythonx		Legacy Vim |python_x| interface. [has-pythonx](undefined#has-pythonx)
			sun		SunOS system.
			ttyin		input is a terminal (tty).
			ttyout		output is a terminal (tty).
			unix		Unix system.
### <a id="vim_starting	True during [startup](undefined#startup)." class="section-title" href="#vim_starting	True during [startup](undefined#startup).">Note:</a>
...
### <a id="has-patch" class="section-title" href="#has-patch">Note:</a>
		3.  Vim patch. For example the "patch123" feature means that
			...
			:if v:version > 602 [](undefined#) v:version == 602 && has("patch148")
...
### <a id="has_key()" class="section-title" href="#has_key()">has_key({dict}, {key})</a>
		The result is a Number, which is TRUE if [Dictionary](undefined#Dictionary) {dict}
...
### <a id="hasmapto()" class="section-title" href="#hasmapto()">hasmapto({what} [, {mode} [, {abbr}]])</a>
		...
		When {abbr} is there and it is [TRUE](undefined#TRUE) use abbreviations
...
### <a id="histdel()" class="section-title" href="#histdel()">histdel({history} [, {item}])</a>
		Clear {history}, i.e. delete all its entries.  See [hist-names](undefined#hist-names)
...
### <a id="histget()" class="section-title" href="#histget()">histget({history} [, {index}])</a>
		...
		{history}.  See [hist-names](undefined#hist-names) for the possible values of
...
### <a id="histnr()" class="section-title" href="#histnr()">histnr({history})</a>
		...
		See [hist-names](undefined#hist-names) for the possible values of {history}.
...
### <a id="index()" class="section-title" href="#index()">index({object}, {expr} [, {start} [, {ic}]])</a>
		If {object} is a [List](undefined#List) return the lowest index where the item
		has a value equal to {expr}.  There is no automatic
		conversion, so the String "4" is different from the Number 4.
		And the Number 4 is different from the Float 4.0.  The value
		of 'ignorecase' is not used here, case always matters.

		If {object} is a [Blob](undefined#Blob) return the lowest index where the byte
		value is equal to {expr}.

		If {start} is given then start looking at the item with index
		{start} (may be negative for an item relative to the end).
		When {ic} is given and it is [TRUE](undefined#TRUE), ignore case.  Otherwise
		case must match.
...
input({opts})
		...
		cancelreturn  ""       The value returned when the dialog is
		                       cancelled.
		highlight     nothing  Highlight handler: [Funcref](undefined#Funcref).
...
### <a id="inputlist()" class="section-title" href="#inputlist()">inputlist({textlist})</a>
		{textlist} must be a [List](undefined#List) of strings.  This [List](undefined#List) is
...
### <a id="asterisks ("") thereby keeping the entry secret, and" class="section-title" href="#asterisks ("") thereby keeping the entry secret, and">Note:</a>
		...
		[history](undefined#history) stack.
...
### <a id="insert()" class="section-title" href="#insert()">insert({object}, {item} [, {idx}])</a>
		When {object} is a [List](undefined#List) or a [Blob](undefined#Blob) insert {item} at the start
		of it.

		If {idx} is specified insert {item} before the item with index
		{idx}.  If {idx} is zero it goes before the first item, just
		like omitting {idx}.  A negative {idx} is also possible, see
		[list-index](undefined#list-index).  -1 inserts just before the last item.

		Returns the resulting [List](undefined#List) or [Blob](undefined#Blob).  Examples:
			:let mylist = insert([2, 3, 5], 1)
			:call insert(mylist, 4, -1)
			:call insert(mylist, 6, len(mylist))
		The last example can be done simpler with |add()|.
		Note that when {item} is a [List](undefined#List) it is inserted as a single
		item.  Use |extend()| to concatenate [Lists](undefined#Lists).
...
### <a id="isdirectory()" class="section-title" href="#isdirectory()">isdirectory({directory})</a>
		The result is a Number, which is [TRUE](undefined#TRUE) when a directory
		with the name {directory} exists.  If {directory} doesn't
		exist, or isn't a directory, the result is [FALSE](undefined#FALSE).  {directory}
...
### <a id="islocked() E786" class="section-title" href="#islocked() E786">islocked({expr})</a>
		The result is a Number, which is [TRUE](undefined#TRUE) when {expr} is the
		name of a locked variable.
		The string argument {expr} must be the name of a variable,
		[List](undefined#List) item or [Dictionary](undefined#Dictionary) entry, not the variable itself!
		Example:
			:let alist = [0, ['a', 'b'], 2, 3]
			:lockvar 1 alist
...
### <a id="id()" class="section-title" href="#id()">id({expr})</a>
		Returns a [String](undefined#String) which is a unique identifier of the
		container type ([List](undefined#List), [Dict](undefined#Dict), [Blob](undefined#Blob) and [Partial](undefined#Partial)). It is
...
### <a id="items()" class="section-title" href="#items()">items({dict})</a>
		Return a [List](undefined#List) with all the key-value pairs of {dict}.  Each
		[List](undefined#List) item is a list with two items: the key of a {dict}
		entry and the value of this entry.  The [List](undefined#List) is in arbitrary
...
### <a id="isnan()" class="section-title" href="#isnan()">isnan({expr})</a>
		Return [TRUE](undefined#TRUE) if {expr} is a float with value NaN.
			echo isnan(0.0 / 0.0)
			1
...
### <a id="jobpid()" class="section-title" href="#jobpid()">jobpid({job})</a>
		Return the PID (process id) of [job-id](undefined#job-id) {job}.

### <a id="jobresize()" class="section-title" href="#jobresize()">jobresize({job}, {width}, {height})</a>
		Resize the pseudo terminal window of [job-id](undefined#job-id) {job} to {width}
...
### <a id="jobstart()" class="section-title" href="#jobstart()">jobstart({cmd} [, {opts}])</a>
		...
		(See [shell-unquoting](undefined#shell-unquoting) for details.)

		Example:
		  :call jobstart('nvim -h', {'on_stdout':{j,d,e->append(line('.'),d)}})
```

		Returns [job-id](undefined#job-id) on success, 0 on invalid arguments (or job
		table is full), -1 if {cmd}[0] or 'shell' is not executable.
		The returned job-id is a valid [channel-id](undefined#channel-id) representing the
		job's stdio streams. Use |chansend()| (or |rpcnotify()| and
		|rpcrequest()| if "rpc" was enabled) to send data to stdin and
		|chanclose()| to close the streams without stopping the job.

		See [job-control](undefined#job-control) and [RPC](undefined#RPC).
...
### <a id="jobstart-options" class="section-title" href="#jobstart-options">Note:</a>
		{opts} is a dictionary with these keys:
		  ...
		  cwd:	      (string, default=[current-directory](/neovim-docs-web/en/vim/editing#current-directory)) Working
		  ...
		  env:	      (dict) Map of environment variable name:value
			      ...
			      the current environment. [jobstart-env](undefined#jobstart-env)
		  ...
		  pty:	      (boolean) Connect the job to a new pseudo
			      ...
			      `on_stderr` is ignored. [terminal-start](/neovim-docs-web/en/neovim/nvim_terminal_emulator#terminal-start)
		  rpc:	      (boolean) Use [msgpack-rpc](undefined#msgpack-rpc) to communicate with
			      the job over stdio. Then `on_stdout` is ignored,
			      but `on_stderr` can still be used.
		  stderr_buffered: (boolean) Collect data until EOF (stream closed)
			      before invoking `on_stderr`. [channel-buffered](undefined#channel-buffered)
		  stdout_buffered: (boolean) Collect data until EOF (stream
			      closed) before invoking `on_stdout`. [channel-buffered](undefined#channel-buffered)
		  stdin:      (string) Either "pipe" (default) to connect the
			      job's stdin to a channel or "null" to disconnect
			      stdin.
		  width:      (number) Width of the `pty` terminal.

		{opts} is passed as [self](undefined#self) dictionary to the callback; the
		caller may set other keys to pass application-specific data.

		Returns:
		  - [channel-id](undefined#channel-id) on success
		  - 0 on invalid arguments
		  - -1 if {cmd}[0] is not executable.
		See also [job-control](undefined#job-control), [channel](undefined#channel), [msgpack-rpc](undefined#msgpack-rpc).

### <a id="jobstop()" class="section-title" href="#jobstop()">jobstop({id})</a>
		Stop [job-id](undefined#job-id) {id} by sending SIGTERM to the job process. If
		the process does not terminate after a timeout then SIGKILL
		will be sent. When the job terminates its |on_exit| handler
		(if any) will be invoked.
		See [job-control](undefined#job-control).
...
### <a id="jobwait()" class="section-title" href="#jobwait()">jobwait({jobs} [, {timeout}])</a>
		...
		{jobs} is a List of [job-id](undefined#job-id)s to wait for.
...
```
		...
		the status of the corresponding job:
			...
			-2 if the job was interrupted (by [CTRL-C](undefined#CTRL-C))
...
```			let lines = join(mylist, "\n") .. "\n"
		String items are used as-is.  [Lists](undefined#Lists) and [Dictionaries](undefined#Dictionaries) are
...
### <a id="json_decode()" class="section-title" href="#json_decode()">json_decode({expr})</a>
		...
		[msgpack-special-dict](undefined#msgpack-special-dict):
...
### <a id="json_encode()" class="section-title" href="#json_encode()">json_encode({expr})</a>
		...
		[msgpack-special-dict](undefined#msgpack-special-dict) as the input.  Will not convert
		[Funcref](undefined#Funcref)s, mappings with non-string keys (can be created as
		[msgpack-special-dict](undefined#msgpack-special-dict)), values with self-referencing
		containers, strings which contain non-UTF-8 characters,
		pseudo-UTF-8 strings which contain codepoints reserved for
		surrogate pairs (such strings are not valid UTF-8 strings).
		Non-printable characters are converted into "\u1234" escapes
		or special escapes like "\t", other are dumped as-is.
		[Blob](undefined#Blob)s are converted to arrays of the individual bytes.

		Can also be used as a [method](undefined#method):
			GetObject()->json_encode()

### <a id="keys()" class="section-title" href="#keys()">keys({dict})</a>
		Return a [List](undefined#List) with all the keys of {dict}.  The [List](undefined#List) is in
...
len({expr})	The result is a Number, which is the length of the argument.
		...
		used, as with |strlen()|.
		When {expr} is a [List](undefined#List) the number of items in the [List](undefined#List) is
		returned.
		When {expr} is a [Blob](undefined#Blob) the number of bytes is returned.
		When {expr} is a [Dictionary](undefined#Dictionary) the number of entries in the
		[Dictionary](undefined#Dictionary) is returned.
...
```
		...
		[last-position-jump](undefined#last-position-jump).
...
### <a id="line2byte()" class="section-title" href="#line2byte()">line2byte({lnum})</a>
		...
		Also see |byte2line()|, [go](undefined#go) and |:goto|.
...
### <a id="log()" class="section-title" href="#log()">log({expr})</a>
		Return the natural logarithm (base e) of {expr} as a [Float](undefined#Float).
		{expr} must evaluate to a [Float](undefined#Float) or a [Number](undefined#Number) in the range
		(0, inf].
		Returns 0.0 if {expr} is not a [Float](undefined#Float) or a [Number](undefined#Number).
		Examples:
...
### <a id="log10()" class="section-title" href="#log10()">log10({expr})</a>
		Return the logarithm of Float {expr} to base 10 as a [Float](undefined#Float).
		{expr} must evaluate to a [Float](undefined#Float) or a [Number](undefined#Number).
		Returns 0.0 if {expr} is not a [Float](undefined#Float) or a [Number](undefined#Number).
		Examples:
			:echo log10(1000)
...
luaeval({expr} [, {expr}])
		...
		to Vim data structures. See [lua-eval](undefined#lua-eval) for more details.

		Can also be used as a [method](undefined#method):
			GetExpr()->luaeval()

### <a id="map()" class="section-title" href="#map()">map({expr1}, {expr2})</a>
		{expr1} must be a [List](undefined#List), [Blob](undefined#Blob) or [Dictionary](undefined#Dictionary).
		Replace each item in {expr1} with the result of evaluating
		{expr2}.  For a [Blob](undefined#Blob) each byte is replaced.
		...
		used as an expression again.  Often it is good to use a
		[literal-string](undefined#literal-string) to avoid having to double backslashes.  You
		still have to double ' quotes

		If {expr2} is a [Funcref](undefined#Funcref) it is called with two arguments:
			1. The key or the index of the current item.
			2. the value of the current item.
...
### <a id="maparg()" class="section-title" href="#maparg()">maparg({name} [, {mode} [, {abbr} [, {dict}]]])</a>
		...
		{mode} can be one of these strings:
			...
			"s"	Select
			"x"	Visual
			"l"	langmap [language-mapping](undefined#language-mapping)
			"t"	Terminal
		...
		When {dict} is there and it is [TRUE](undefined#TRUE) return a dictionary
		...
		following items:
		  ...
		  "mode"     Modes for which the mapping is defined. In
			     ...
			     "!"     Insert and Commandline mode
				     ([mapmode-ic](undefined#mapmode-ic))
...
### <a id="|maparg()|. E460" class="section-title" href="#|maparg()|. E460">Note:</a>
...
### <a id="match()" class="section-title" href="#match()">match({expr}, {pat} [, {start} [, {count}]])</a>
		When {expr} is a [List](undefined#List) then this returns the index of the
		first item where {pat} matches.  Each item is used as a
		String, [Lists](undefined#Lists) and [Dictionaries](undefined#Dictionaries) are used as echoed.

		Otherwise, {expr} is used as a String.  The result is a
		Number, which gives the index (byte offset) in {expr} where
		{pat} matches.

		A match at the first character or [List](undefined#List) item returns zero.
		...
		See [string-match](/neovim-docs-web/en/vim/builtin#string-match) for how {pat} is used.
...
```
		...
		{start} in a String or item {start} in a [List](undefined#List).
...
```			:echo match("testing", "ing", 2)
		...
		String or {start} > len({expr}) for a [List](undefined#List)) -1 is returned.

		When {count} is given use the {count}'th match.  When a match
		is found in a String the search for the next one starts one
		character further.  Thus this example results in 1:
			echo match("testing", "..", 0, 2)
		In a [List](undefined#List) the search continues in the next item.
		Note that when {count} is added the way {start} works changes,
		see above.

		See [pattern](undefined#pattern) for the patterns that are accepted.
...
matchadd({group}, {pattern} [, {priority} [, {id} [, {dict}]]])
		...
		respectively.  3 is reserved for use by the [matchparen](undefined#matchparen)
		plugin.
		...
		conceal character that will be shown for [hl-Conceal](undefined#hl-Conceal)
		highlighted matches. The dict can have the following members:

			conceal	    Special character to show instead of the
				    match (only for [hl-Conceal](undefined#hl-Conceal) highlighted
...
### <a id="matcharg()" class="section-title" href="#matcharg()">matcharg({nr})</a>
		...
		Return a [List](undefined#List) with two elements:
			The name of the highlight group used
			The pattern used.
		When {nr} is not 1, 2 or 3 returns an empty [List](undefined#List).
...
### <a id="strspn() strcspn()" class="section-title" href="#strspn() strcspn()">Note:</a>
		...
		result is "-1".
		When {expr} is a [List](undefined#List) the result is equal to |match()|.

		Can also be used as a [method](undefined#method):
			GetText()->matchend('word')

### <a id="matchfuzzy()" class="section-title" href="#matchfuzzy()">matchfuzzy({list}, {str} [, {dict}])</a>
		If {list} is a list of strings, then returns a [List](undefined#List) with all
		...
		argument supports the following additional items:
		    key		Key of the item which is fuzzy matched against
				...
				string.
		    text_cb	[Funcref](undefined#Funcref) that will be called for every item
		...
		Refer to [fuzzy-matching](/neovim-docs-web/en/vim/pattern#fuzzy-matching) for more information about fuzzy
		matching strings.
...
### <a id="matchlist()" class="section-title" href="#matchlist()">matchlist({expr}, {pat} [, {start} [, {count}]])</a>
		Same as |match()|, but return a [List](undefined#List).  The first item in the
...
### <a id="matchstr()" class="section-title" href="#matchstr()">matchstr({expr}, {pat} [, {start} [, {count}]])</a>
		...
		result is "".
		When {expr} is a [List](undefined#List) then the matching item is returned.
...
### <a id="matchstrpos()" class="section-title" href="#matchstrpos()">matchstrpos({expr}, {pat} [, {start} [, {count}]])</a>
		...
		result is ["", -1, -1].
		When {expr} is a [List](undefined#List) then the matching item, the index
...
```			echo max([apples, pears, oranges])
		...
		{expr} can be a [List](undefined#List) or a [Dictionary](undefined#Dictionary).  For a Dictionary,
		it returns the maximum of all values in the Dictionary.
		If {expr} is neither a List nor a Dictionary, or one of the
		items in {expr} cannot be used as a Number this results in
                an error.  An empty [List](undefined#List) or [Dictionary](undefined#Dictionary) results in zero.
...
### <a id="menu_get()" class="section-title" href="#menu_get()">menu_get({path} [, {modes}])</a>
		Returns a [List](undefined#List) of [Dictionaries](undefined#Dictionaries) describing [menus](/neovim-docs-web/en/vim/gui#menus) (defined
		by |:menu|, |:amenu|, …), including [hidden-menus](undefined#hidden-menus).
...
```
		...
		[creating-menus](undefined#creating-menus) for the list of modes). "a" means "all".
...
### <a id="menu_info()" class="section-title" href="#menu_info()">menu_info({name} [, {mode}])</a>
		...
		Returns a [Dictionary](undefined#Dictionary) containing the following items:
		  accel		menu item accelerator text [menu-text](undefined#menu-text)
		  display	display name (name without '&')
		  enabled	v:true if this menu item is enabled
				Refer to |:menu-enable|
		  icon		name of the icon file (for toolbar)
				[toolbar-icon](undefined#toolbar-icon)
		  ...
		  priority	menu order priority [menu-priority](undefined#menu-priority)
		  ...
		  shortcut	shortcut key (character after '&' in
				the menu name) [menu-shortcut](undefined#menu-shortcut)
		  silent	v:true if the menu item is created
				with <silent> argument |:menu-silent|
		  submenus	[List](undefined#List) containing the names of
...
mode([expr])	Return a string that indicates the current mode.
		...
		a non-empty String ([non-zero-arg](undefined#non-zero-arg)), then the full mode is
		returned, otherwise only the first letter is returned.
		   ...
		   niI	    Normal using |i_CTRL-O| in [Insert-mode](undefined#Insert-mode)
		   niR	    Normal using |i_CTRL-O| in [Replace-mode](undefined#Replace-mode)
		   niV	    Normal using |i_CTRL-O| in [Virtual-Replace-mode](undefined#Virtual-Replace-mode)
		   nt	    Normal in [terminal-emulator](undefined#terminal-emulator) (insert goes to
				Terminal mode)
		   ntT	    Normal using |t_CTRL-\_CTRL-O| in [Terminal-mode](undefined#Terminal-mode)
		   ...
		   i	    Insert
		   ic	    Insert mode completion [compl-generic](undefined#compl-generic)
		   ix	    Insert mode |i_CTRL-X| completion
		   R	    Replace [R](undefined#R)
		   Rc	    Replace mode completion [compl-generic](undefined#compl-generic)
		   Rx	    Replace mode |i_CTRL-X| completion
		   Rv	    Virtual Replace [gR](undefined#gR)
		   Rvc	    Virtual Replace mode completion [compl-generic](undefined#compl-generic)
		   Rvx	    Virtual Replace mode |i_CTRL-X| completion
		   c	    Command-line editing
		   cv	    Vim Ex mode [gQ](undefined#gQ)
		   r	    Hit-enter prompt
		   rm	    The -- more -- prompt
...
### <a id="msgpackdump()" class="section-title" href="#msgpackdump()">msgpackdump({list} [, {type}])</a>
		...
		|readfile()|-style list. When {type} contains "B", a [Blob](undefined#Blob) is
		returned instead. Example:
			call writefile(msgpackdump([{}]), 'fname.mpack', 'b')
		or, using a [Blob](undefined#Blob):
...
```
		...
		1. [Funcref](undefined#Funcref)s cannot be dumped.
		2. Containers that reference themselves cannot be dumped.
		3. Dictionary keys are always dumped as STR strings.
		4. Other strings and [Blob](undefined#Blob)s are always dumped as BIN strings.
		5. Points 3. and 4. do not apply to [msgpack-special-dict](undefined#msgpack-special-dict)s.

### <a id="msgpackparse()" class="section-title" href="#msgpackparse()">msgpackparse({data})</a>
		Convert a |readfile()|-style list or a [Blob](undefined#Blob) to a list of
		VimL objects.
		Example:
```			let fname = expand('~/.config/nvim/shada/main.shada')
		...
		1. Mapping ordering is not preserved unless messagepack
		   ...
		   ([msgpack-special-map](undefined#msgpack-special-map)).
		2. Since the parser aims to preserve all data untouched
		   (except for 1.) some strings are parsed to
		   [msgpack-special-dict](undefined#msgpack-special-dict) format which is not convenient to
		   use.
### <a id="msgpack-special-dict" class="section-title" href="#msgpack-special-dict">Note:</a>
		...
		boolean	One or zero.  When dumping it is only checked that
			value is a [Number](undefined#Number).  Not returned by |msgpackparse()|
			since |v:true| and |v:false| were introduced.
		integer	[List](undefined#List) with four numbers: sign (-1 or 1), highest two
...
### <a id="_VAL[0]  ((_VAL[1] << 62)" class="section-title" href="#_VAL[0]  ((_VAL[1] << 62)">Note:</a>
			...
			circumstances:
			1. [Number](undefined#Number) is 32-bit and value is either above
			   INT32_MAX or below INT32_MIN.
			2. [Number](undefined#Number) is 64-bit and value is above INT64_MAX. It
			   cannot possibly be below INT64_MIN because msgpack
			   C parser does not support such values.
		float	[Float](undefined#Float). This value cannot possibly appear in
			|msgpackparse()| output.
		string	|readfile()|-style list of strings. This value will
			...
			reasons.
		binary	[String](undefined#String), or [Blob](undefined#Blob) if binary string contains zero
			byte. This value cannot appear in |msgpackparse()|
			output since blobs were introduced.
		array	[List](undefined#List). This value cannot appear in |msgpackparse()|
			output.
### <a id="msgpack-special-map" class="section-title" href="#msgpack-special-map">Note:</a>
		map	[List](undefined#List) of [List](undefined#List)s with two items (key and value) each.
			...
			3. Duplicate key.
			4. Empty key.
		ext	[List](undefined#List) with two values: first is a signed integer
...
### <a id="E5555 nvim_...() eval-api" class="section-title" href="#E5555 nvim_...() eval-api">nvim_...({...})</a>
		Call nvim [api](undefined#api) functions. The type checking of arguments will
		be stricter than for most other builtins. For instance,
		if Integer is expected, a [Number](undefined#Number) must be passed in, a
		[String](undefined#String) will not be autoconverted.
...
### <a id="perleval()" class="section-title" href="#perleval()">perleval({expr})</a>
		Evaluate [perl](undefined#perl) expression {expr} and return its result
		converted to Vim data structures.
		Numbers and strings are returned as they are (strings are
		copied though).
		Lists are represented as Vim [List](undefined#List) type.
		Dictionaries are represented as Vim [Dictionary](undefined#Dictionary) type,
...
### <a id="pow()" class="section-title" href="#pow()">pow({x}, {y})</a>
		Return the power of {x} to the exponent {y} as a [Float](undefined#Float).
		{x} and {y} must evaluate to a [Float](undefined#Float) or a [Number](undefined#Number).
		Returns 0.0 if {x} or {y} is not a [Float](undefined#Float) or a [Number](undefined#Number).
		Examples:
			:echo pow(3, 3)
			27.0
			:echo pow(2, 16)
...
### <a id="printf()" class="section-title" href="#printf()">printf({fmt}, {expr1} ...)</a>
		...
		When used as a [method](undefined#method) the base is passed as the second
		argument:
...
### <a id="prompt_getprompt()" class="section-title" href="#prompt_getprompt()">prompt_getprompt({buf})</a>
		...
		be a buffer name or number.  See [prompt-buffer](/neovim-docs-web/en/neovim/channel#prompt-buffer).
...
### <a id="prompt_setcallback()" class="section-title" href="#prompt_setcallback()">prompt_setcallback({buf}, {expr})</a>
		...
		Example:
		   ...
		   func s:TextEntered(text)
		     if a:text == 'exit' [](undefined#) a:text == 'quit'
		       stopinsert
		       close
		     else
...
### <a id="pum_getpos()" class="section-title" href="#pum_getpos()">pum_getpos()</a>
		If the popup menu (see [ins-completion-menu](undefined#ins-completion-menu)) is not visible,
		returns an empty [Dictionary](undefined#Dictionary), otherwise, returns a
		[Dictionary](undefined#Dictionary) with the following keys:
			height		nr of items visible
			width		screen cells
			row		top screen row (0 first row)
			col		leftmost screen column (0 first col)
			size		total nr of items
			scrollbar	[TRUE](undefined#TRUE) if scrollbar is visible

		The values are the same as in |v:event| during [CompleteChanged](undefined#CompleteChanged).

### <a id="pumvisible()" class="section-title" href="#pumvisible()">pumvisible()</a>
		Returns non-zero when the popup menu is visible, zero
		otherwise.  See [ins-completion-menu](undefined#ins-completion-menu).
...
### <a id="py3eval()" class="section-title" href="#py3eval()">py3eval({expr})</a>
		...
		UTF-8).
		Lists are represented as Vim [List](undefined#List) type.
		Dictionaries are represented as Vim [Dictionary](undefined#Dictionary) type with
...
### <a id="range()" class="section-title" href="#range()">range({expr} [, {max} [, {stride}]])</a>
		Returns a [List](undefined#List) with Numbers:
...
readfile({fname} [, {type} [, {max}]])
		Read file {fname} and return a [List](undefined#List), each line of the file
		...
		When {type} contains "B" a [Blob](undefined#Blob) is returned with the binary
...
### <a id="reduce() E998" class="section-title" href="#reduce() E998">reduce({object}, {func} [, {initial}])</a>
		...
		[List](undefined#List) or a [Blob](undefined#Blob).  {func} is called with two arguments: the
...
### <a id="reg_recorded()" class="section-title" href="#reg_recorded()">reg_recorded()</a>
		...
		See [q](undefined#q) and [Q](undefined#Q).

### <a id="reg_recording()" class="section-title" href="#reg_recording()">reg_recording()</a>
		Returns the single letter name of the register being recorded.
		Returns an empty string when not recording.  See [q](undefined#q).
...
### <a id="reltimefloat()" class="section-title" href="#reltimefloat()">reltimefloat({time})</a>
		...
		Also see [profiling](undefined#profiling).
...
### <a id="remove()" class="section-title" href="#remove()">remove({list}, {idx}, {end})</a>
		Without {end}: Remove the item at {idx} from [List](undefined#List) {list} and
		return the item.
		With {end}: Remove items from {idx} to {end} (inclusive) and
		return a [List](undefined#List) with these items.  When {idx} points to the same
		item as {end} a list with one item is returned.  When {end}
		points to an item before {idx} this is an error.
		See [list-index](undefined#list-index) for possible values of {idx} and {end}.
		Returns zero on error.
		Example:
...
remove({blob}, {idx})
remove({blob}, {idx}, {end})
		Without {end}: Remove the byte at {idx} from [Blob](undefined#Blob) {blob} and
		return the byte.
		With {end}: Remove bytes from {idx} to {end} (inclusive) and
		return a [Blob](undefined#Blob) with these bytes.  When {idx} points to the same
		byte as {end} a [Blob](undefined#Blob) with one byte is returned.  When {end}
...
### <a id="repeat()" class="section-title" href="#repeat()">repeat({expr}, {count})</a>
		...
		When {expr} is a [List](undefined#List) the result is {expr} concatenated
		{count} times.  Example:
...
reverse({object})
		...
		{object} can be a [List](undefined#List) or a [Blob](undefined#Blob).
		Returns {object}.
...
### <a id="round()" class="section-title" href="#round()">round({expr})</a>
		...
		as a [Float](undefined#Float).  If {expr} lies halfway between two integral
...
### <a id="rpcnotify()" class="section-title" href="#rpcnotify()">rpcnotify({channel}, {event} [, {args}...])</a>
		Sends {event} to {channel} via [RPC](undefined#RPC) and returns immediately.
...
### <a id="rpcrequest()" class="section-title" href="#rpcrequest()">rpcrequest({channel}, {method} [, {args}...])</a>
		...
		[RPC](undefined#RPC) and blocks until a response is received.
		Example:
...
### <a id="rubyeval()" class="section-title" href="#rubyeval()">rubyeval({expr})</a>
		...
		are copied though).
		Arrays are represented as Vim [List](undefined#List) type.
		Hashes are represented as Vim [Dictionary](undefined#Dictionary) type.
...
### <a id="screenpos()" class="section-title" href="#screenpos()">screenpos({winid}, {lnum}, {col})</a>
		...
		The [conceal](undefined#conceal) feature is ignored here, the column numbers are
		as if 'conceallevel' is zero.  You can set the cursor to the
		right position and use |screencol()| to get the value with
		[conceal](undefined#conceal) taken into account.
...
search({pattern} [, {flags} [, {stopline} [, {timeout} [, {skip}]]]])
		...
		matters for overlapping matches.  See [cpo-c](undefined#cpo-c).  You can also
...
### <a id="searchcount()" class="section-title" href="#searchcount()">searchcount([{options}])</a>
		...
		previous pattern was not set and "pattern" was not specified.
		  ...
		  key		type		meaning ~
		  current	[Number](undefined#Number)	current position of match;
						0 if the cursor position is
						before the first match
		  exact_match	[Boolean](undefined#Boolean)	1 if "current" is matched on
						"pos", otherwise 0
		  total		[Number](undefined#Number)	total count of matches found
		  incomplete	[Number](undefined#Number)	0: search was fully completed
						1: recomputing was timed out
						2: max count exceeded

		For {options} see further down.

		To get the last search count when [n](undefined#n) or [N](undefined#N) was pressed, call
		this function with `recompute: 0` . This sometimes returns
		wrong information because [n](undefined#n) and [N](undefined#N)'s maximum count is 99.
...
```
		...
		[CursorMoved](undefined#CursorMoved) or [CursorMovedI](undefined#CursorMovedI) autocommand:
```
...
```
		...
		{options} must be a Dictionary. It can contain:
		  key		type		meaning ~
		  recompute	[Boolean](undefined#Boolean)	if [TRUE](undefined#TRUE), recompute the count
						like [n](undefined#n) or [N](undefined#N) was executed.
						otherwise returns the last
						computed result (when [n](undefined#n) or
						[N](undefined#N) was used when "S" is not
						in 'shortmess', or this
						function was called).
						(default: [TRUE](undefined#TRUE))
		  pattern	[String](undefined#String)	recompute if this was given
						and different with |@/|.
...
```						  let @/ = pattern
						(default: |@/|)
		  timeout	[Number](undefined#Number)	0 or negative number is no
						timeout. timeout milliseconds
						for recomputing the result
						(default: 0)
		  maxcount	[Number](undefined#Number)	0 or negative number is no
						...
						becomes `maxcount + 1`
						(default: 0)
		  pos		[List](undefined#List)		`[lnum, col, off]` value
...
### <a id="searchdecl()" class="section-title" href="#searchdecl()">searchdecl({name} [, {global} [, {thisblock}]])</a>
		...
		With a non-zero {global} argument it works like [gD](undefined#gD), find
		first match in the file.  Otherwise it works like [gd](undefined#gd), find
...
searchpair({start}, {middle}, {end} [, {flags} [, {skip}
		...
		{start}, {middle} and {end} are patterns, see [pattern](undefined#pattern).  They
...
searchpairpos({start}, {middle}, {end} [, {flags} [, {skip}
				[, {stopline} [, {timeout}]]]])
		Same as |searchpair()|, but returns a [List](undefined#List) with the line and
		column position of the match. The first element of the [List](undefined#List)
...
```
		...
		See [match-parens](/neovim-docs-web/en/vim/tips#match-parens) for a bigger and more useful example.

### <a id="searchpos()" class="section-title" href="#searchpos()">Note:</a>
searchpos({pattern} [, {flags} [, {stopline} [, {timeout} [, {skip}]]]])
		Same as |search()|, but returns a [List](undefined#List) with the line and
		column position of the match. The first element of the [List](undefined#List)
		is the line number and the second element is the byte index of
		the column position of the match. If no match is found,
		returns [0, 0].
...
### <a id="serverstart()" class="section-title" href="#serverstart()">serverstart([{address}])</a>
		...
		[RPC](undefined#RPC) messages. Clients can send [API](undefined#API) commands to the
...
```			:call setline(5, strftime("%c"))
		...
		When {text} is a [List](undefined#List) then line {lnum} and following lines
...
### <a id="setloclist()" class="section-title" href="#setloclist()">setloclist({nr}, {list} [, {action} [, {what}]])</a>
		...
		{nr} can be the window number or the [window-ID](undefined#window-ID).
		When {nr} is zero the current window is used.

		For a location list window, the displayed location list is
		modified.  For an invalid window number {nr}, -1 is returned.
		Otherwise, same as |setqflist()|.
		Also see [location-list](undefined#location-list).

		For {action} see [setqflist-action](undefined#setqflist-action).
...
setpos({expr}, {list})
		...
		{list} must be a [List](undefined#List) with four or five numbers:
		    [bufnum, lnum, col, off]
		...
		vertically; if you set the cursor position with this, [j](undefined#j) and
		[k](undefined#k) motions will jump to previous columns!  Use |cursor()| to
...
```
		...
		The following items can be specified in dictionary {what}:
		    ...
		    efm		errorformat to use when parsing text from
				...
				See [quickfix-parse](undefined#quickfix-parse)
		    id		quickfix list identifier [quickfix-ID](undefined#quickfix-ID)
		    idx		index of the current entry in the quickfix
				list specified by "id" or "nr". If set to '$',
				then the last entry in the list is set as the
				current entry.  See [quickfix-index](undefined#quickfix-index)
		    items	list of quickfix entries. Same as the {list}
				argument.
		    lines	use 'errorformat' to parse a list of lines and
				add the resulting entries to the quickfix list
				{nr} or {id}.  Only a [List](undefined#List) value is supported.
				See [quickfix-parse](undefined#quickfix-parse)
		    ...
		    quickfixtextfunc
				...
				[quickfix-window-function](/neovim-docs-web/en/vim/quickfix#quickfix-window-function) for an explanation
				of how to write the function and an example.
		    title	quickfix list title text. See [quickfix-title](undefined#quickfix-title)
		...
		Examples (See also [setqflist-examples](undefined#setqflist-examples)):
...
setreg({regname}, {value} [, {options}])
		...
		|getreginfo()|, including a [List](undefined#List) or [Dict](undefined#Dict).
		If {options} contains "a" or {regname} is upper case,
		then the value is appended.

		{options} can also contain a register type specification:
		    "c" or "v"	      [charwise](undefined#charwise) mode
		    "l" or "V"	      [linewise](undefined#linewise) mode
		    "b" or "<CTRL-V>" [blockwise-visual](undefined#blockwise-visual) mode
...
### <a id="E883" class="section-title" href="#E883">Note:</a>
		Note: you may not use [List](undefined#List) containing more than one item to
...
### <a id=":call setreg('', @%, 'ac')" class="section-title" href="#:call setreg('', @%, 'ac')">Note:</a>
		...
		represented as newlines as well, see [NL-used-for-Nul](undefined#NL-used-for-Nul).
...
### <a id="E962" class="section-title" href="#E962">Note:</a>
		...
		Examples (for more examples see [tagstack-examples](undefined#tagstack-examples)):
...
### <a id="shellescape()" class="section-title" href="#shellescape()">shellescape({string} [, {special}])</a>
		...
		If {special} is a [non-zero-arg](undefined#non-zero-arg):
		...
		- The <NL> character is escaped (twice if {special} is
		  a [non-zero-arg](undefined#non-zero-arg)).
...
sign_ functions are documented here: [sign-functions-details](/neovim-docs-web/en/vim/sign#sign-functions-details)
...
### <a id="sin()" class="section-title" href="#sin()">sin({expr})</a>
		Return the sine of {expr}, measured in radians, as a [Float](undefined#Float).
		{expr} must evaluate to a [Float](undefined#Float) or a [Number](undefined#Number).
		Returns 0.0 if {expr} is not a [Float](undefined#Float) or a [Number](undefined#Number).
		Examples:
			:echo sin(100)
...
### <a id="sinh()" class="section-title" href="#sinh()">sinh({expr})</a>
		Return the hyperbolic sine of {expr} as a [Float](undefined#Float) in the range
		[-inf, inf].
		{expr} must evaluate to a [Float](undefined#Float) or a [Number](undefined#Number).
		Returns 0.0 if {expr} is not a [Float](undefined#Float) or a [Number](undefined#Number).
		Examples:
...
### <a id="sockconnect()" class="section-title" href="#sockconnect()">sockconnect({mode}, {address} [, {opts}])</a>
		...
		Returns a [channel](undefined#channel) ID. Close the socket with |chanclose()|.
		...
		{opts} is an optional dictionary with these keys:
		  ...
		  data_buffered : read socket data in [channel-buffered](undefined#channel-buffered) mode.
		  rpc     : If set, [msgpack-rpc](undefined#msgpack-rpc) will be used to communicate
			    over the socket.
		Returns:
...
### <a id="sort() E702" class="section-title" href="#sort() E702">sort({list} [, {func} [, {dict}]])</a>
		...
		after Strings, [Lists](undefined#Lists) after Numbers.  For sorting text in the
		...
		When {func} is a [Funcref](undefined#Funcref) or a function name, this function
...
spellsuggest({word} [, {max} [, {capital}]])
		Return a [List](undefined#List) with spelling suggestions to replace {word}.
...
### <a id="split()" class="section-title" href="#split()">split({string} [, {pattern} [, {keepempty}]])</a>
		Make a [List](undefined#List) out of {string}.  When {pattern} is omitted or
...
### <a id="sqrt()" class="section-title" href="#sqrt()">sqrt({expr})</a>
		...
		{expr} must evaluate to a [Float](undefined#Float) or a [Number](undefined#Number).  When {expr}
		is negative the result is NaN (Not a Number).  Returns 0.0 if
		{expr} is not a [Float](undefined#Float) or a [Number](undefined#Number).
		Examples:
			:echo sqrt(100)
			10.0
			:echo sqrt(-4.01)
...
### <a id="stdioopen()" class="section-title" href="#stdioopen()">stdioopen({opts})</a>
		With [--headless](undefined#--headless) this opens stdin and stdout as a [channel](undefined#channel).
		May be called only once. See [channel-stdio](/neovim-docs-web/en/neovim/channel#channel-stdio). stderr is not
		...
		{opts} is a dictionary with these keys:
		  ...
		  on_print : callback invoked when Nvim needs to print a
			     ...
			     as sole argument.
		  stdin_buffered : read stdin in [channel-buffered](undefined#channel-buffered) mode.
		  rpc      : If set, [msgpack-rpc](undefined#msgpack-rpc) will be used to communicate
			     over stdio
		Returns:
		  - [channel-id](undefined#channel-id) on success (value is always 1)
		  - 0 on invalid arguments


### <a id="stdpath() E6100" class="section-title" href="#stdpath() E6100">stdpath({what})</a>
		Returns [standard-path](/neovim-docs-web/en/vim/starting#standard-path) locations of various default files and
		directories.
		...
		state        String  Session state directory: storage for file
				     drafts, swap, undo, [shada](undefined#shada).
...
### <a id="str2float()" class="section-title" href="#str2float()">str2float({string} [, {quoted}])</a>
		...
		see [floating-point-format](undefined#floating-point-format).  But it's a bit more permissive.
...
### <a id="submatch() E935" class="section-title" href="#submatch() E935">submatch({nr} [, {list}])</a>
		...
		Also see [sub-replace-expression](undefined#sub-replace-expression).
...
### <a id="substitute()" class="section-title" href="#substitute()">substitute({string}, {pat}, {sub}, {flags})</a>
		...
		'smartcase' is not used.  See [string-match](/neovim-docs-web/en/vim/builtin#string-match) for how {pat} is
		used.

		A "~" in {sub} is not replaced with the previous {sub}.
		Note that some codes in {sub} have a special meaning
		[sub-replace-special](undefined#sub-replace-special).  For example, to replace something with
		"\n" (two characters), use "\\\\n" or '\\n'.
...
### <a id=":echo substitute("testing", ".", "\\U\\0", "")" class="section-title" href="#:echo substitute("testing", ".", "\\U\\0", "")">Note:</a>
		...
		an expression. See [sub-replace-expression](undefined#sub-replace-expression).  Example:
...
### <a id="synID()" class="section-title" href="#synID()">synID({lnum}, {col}, {trans})</a>
		...
		When {trans} is [TRUE](undefined#TRUE), transparent items are reduced to the
		item that they reveal.  This is useful when wanting to know
		the effective color.  When {trans} is [FALSE](undefined#FALSE), the transparent
...
### <a id="synIDattr()" class="section-title" href="#synIDattr()">synIDattr({synID}, {what} [, {mode}])</a>
		...
		"font"		font name (only available in the GUI)
				[highlight-font](undefined#highlight-font)
		"sp"		special color (as with "fg") [guisp](undefined#guisp)
...
### <a id="synconcealed()" class="section-title" href="#synconcealed()">synconcealed({lnum}, {col})</a>
		The result is a [List](undefined#List) with currently three items:
...
### <a id="synstack()" class="section-title" href="#synstack()">synstack({lnum}, {col})</a>
		Return a [List](undefined#List), which is the stack of syntax items at the
...
### <a id="system() E677" class="section-title" href="#system() E677">system({cmd} [, {input}])</a>
		Gets the output of {cmd} as a [string](undefined#string) (|systemlist()| returns
		a [List](undefined#List)) and sets |v:shell_error| to the error code.
		...
		If {input} is a [List](undefined#List) it is written to the pipe as
...
### <a id="systemlist()" class="section-title" href="#systemlist()">systemlist({cmd} [, {input} [, {keepempty}]])</a>
		Same as |system()|, but returns a [List](undefined#List) with lines (parts of
...
### <a id="tabpagebuflist()" class="section-title" href="#tabpagebuflist()">tabpagebuflist([{arg}])</a>
		The result is a [List](undefined#List), where each item is the number of the
...
tagfiles()	Returns a [List](undefined#List) with the file names used to search for tags
		for the current buffer.  This is the 'tags' option expanded.


### <a id="taglist()" class="section-title" href="#taglist()">taglist({expr} [, {filename}])</a>
		Returns a [List](undefined#List) of tags matching the regular expression {expr}.

		If {filename} is passed it is used to prioritize the results
		in the same way that |:tselect| does. See [tag-priority](undefined#tag-priority).
		...
		entries:
			...
			static		A file specific tag.  Refer to
					[static-tag](undefined#static-tag) for more information.
		...
		Refer to [tag-regexp](undefined#tag-regexp) for more information about the tag
		search regular expression pattern.

		Refer to |'tags'| for information about how the tags file is
		located by Vim. Refer to [tags-file-format](undefined#tags-file-format) for the format of
...
### <a id="tempname() temp-file-name" class="section-title" href="#tempname() temp-file-name">tempname()</a>
		...
		doesn't exist.  It can be used for a temporary file.  Example:
			...
			:exe "redir > " .. tmpfile
		For Unix, the file will be in a private directory [tempfile](undefined#tempfile).
...
### <a id="termopen()" class="section-title" href="#termopen()">termopen({cmd} [, {opts}])</a>
		...
		Terminal environment is initialized as in [](undefined#)jobstart-env|,
		except $TERM is set to "xterm-256color". Full behavior is
		described in [terminal](undefined#terminal).

### <a id="tan()" class="section-title" href="#tan()">tan({expr})</a>
		Return the tangent of {expr}, measured in radians, as a [Float](undefined#Float)
...
### <a id="tanh()" class="section-title" href="#tanh()">tanh({expr})</a>
		Return the hyperbolic tangent of {expr} as a [Float](undefined#Float) in the
		range [-1, 1].
		{expr} must evaluate to a [Float](undefined#Float) or a [Number](undefined#Number).
		Returns 0.0 if {expr} is not a [Float](undefined#Float) or a [Number](undefined#Number).
		Examples:
...
timer_info([{id}])
		...
		For each timer the information is stored in a [Dictionary](undefined#Dictionary) with
		these items:
		    "id"	    the timer ID
...
### <a id="timer_pause()" class="section-title" href="#timer_pause()">timer_pause({timer}, {paused})</a>
		...
		See [non-zero-arg](undefined#non-zero-arg).
...
timer_start({time}, {callback} [, {options}])
		...
		function or a [Funcref](undefined#Funcref).  It is called with one argument, which
...
```			func MyHandler(timer)
		...
		Not available in the [sandbox](undefined#sandbox).
...
### <a id="tolower()" class="section-title" href="#tolower()">tolower({expr})</a>
		...
		characters turned into lowercase (just like applying [gu](undefined#gu) to
...
### <a id="toupper()" class="section-title" href="#toupper()">toupper({expr})</a>
		...
		characters turned into uppercase (just like applying [gU](undefined#gU) to
		the string).  Returns an empty string on error.
...
### <a id="trunc()" class="section-title" href="#trunc()">trunc({expr})</a>
		...
		equal to {expr} as a [Float](undefined#Float) (truncate towards zero).
		{expr} must evaluate to a [Float](undefined#Float) or a [Number](undefined#Number).
		Returns 0.0 if {expr} is not a [Float](undefined#Float) or a [Number](undefined#Number).
		Examples:
			echo trunc(1.456)
...
### <a id="undotree()" class="section-title" href="#undotree()">undotree()</a>
		...
		the following items:
		  ...
		  "synced"	Non-zero when the last undo block was synced.
				...
				user.  See [undo-blocks](/neovim-docs-web/en/vim/undo#undo-blocks).
		  "entries"	A list of dictionaries with information about
				undo blocks.

		The first item in the "entries" list is the oldest undo item.
		Each List item is a [Dictionary](undefined#Dictionary) with these items:
...
### <a id="uniq() E882" class="section-title" href="#uniq() E882">uniq({list} [, {func} [, {dict}]])</a>
		...
		Returns zero if {list} is not a [List](undefined#List).

		Can also be used as a [method](undefined#method):
			mylist->uniq()

### <a id="values()" class="section-title" href="#values()">values({dict})</a>
		Return a [List](undefined#List) with all the values of {dict}.  The [List](undefined#List) is
		in arbitrary order.  Also see |items()| and |keys()|.
		Returns zero if {dict} is not a [Dict](undefined#Dict).
...
### <a id="virtcol()" class="section-title" href="#virtcol()">virtcol({expr})</a>
		...
		set to 8, it returns 8. [conceal](undefined#conceal) is ignored.
...
### <a id="virtcol2col()" class="section-title" href="#virtcol2col()">virtcol2col({winid}, {lnum}, {col})</a>
		...
		[window-ID](undefined#window-ID). If this is zero, then the current window is used.
...
### <a id="visualmode()" class="section-title" href="#visualmode()">visualmode([{expr}])</a>
		...
		the old value is returned.  See [non-zero-arg](undefined#non-zero-arg).

### <a id="wait()" class="section-title" href="#wait()">wait({timeout}, {condition} [, {interval}])</a>
		Waits until {condition} evaluates to [TRUE](undefined#TRUE), where {condition}
		is a [Funcref](undefined#Funcref) or [string](undefined#string) containing an expression.
		...
		Returns a status integer:
			...
			-1 if the timeout was exceeded
			-2 if the function was interrupted (by [CTRL-C](undefined#CTRL-C))
			-3 if an error occurred

### <a id="wildmenumode()" class="section-title" href="#wildmenumode()">wildmenumode()</a>
		Returns [TRUE](undefined#TRUE) when the wildmenu is active and [FALSE](undefined#FALSE)
		otherwise.  See 'wildmenu' and 'wildmode'.
		This can be used in mappings to handle the 'wildcharm' option
		gracefully. (Makes only sense with [mapmode-c](undefined#mapmode-c) mappings).
...
### <a id="win_findbuf()" class="section-title" href="#win_findbuf()">win_findbuf({bufnr})</a>
		Returns a [List](undefined#List) with [window-ID](undefined#window-ID)s for windows that contain
...
### <a id="win_getid()" class="section-title" href="#win_getid()">win_getid([{win} [, {tab}]])</a>
		Get the [window-ID](undefined#window-ID) for the specified window.
...
### <a id="win_gettype()" class="section-title" href="#win_gettype()">win_gettype([{nr}])</a>
		Return the type of the window:
			...
			"command"	command-line window [cmdwin](undefined#cmdwin)
			(empty)		normal window
			"loclist"	[location-list-window](undefined#location-list-window)
			"popup"		floating window [api-floatwin](/neovim-docs-web/en/neovim/api#api-floatwin)
			"preview"	preview window [preview-window](/neovim-docs-web/en/vim/windows#preview-window)
			"quickfix"	[quickfix-window](/neovim-docs-web/en/vim/quickfix#quickfix-window)
...
### <a id="win_move_separator()" class="section-title" href="#win_move_separator()">win_move_separator({nr}, {offset})</a>
		...
		can be a window number or [window-ID](undefined#window-ID). A positive {offset}
...
### <a id="win_move_statusline()" class="section-title" href="#win_move_statusline()">win_move_statusline({nr}, {offset})</a>
		...
		window number or [window-ID](undefined#window-ID). A positive {offset} moves down
...
### <a id="win_screenpos()" class="section-title" href="#win_screenpos()">win_screenpos({nr})</a>
		...
		{nr} can be the window number or the [window-ID](undefined#window-ID).  Use zero
...
### <a id="win_splitmove()" class="section-title" href="#win_splitmove()">win_splitmove({nr}, {target} [, {options}])</a>
		...
		Both {nr} and {target} can be window numbers or [window-ID](undefined#window-ID)s.
		Both must be in the current tab page.

		Returns zero for success, non-zero for failure.

		{options} is a [Dictionary](undefined#Dictionary) with the following optional entries:
...
winbufnr({nr})	The result is a Number, which is the number of the buffer
		...
		the [window-ID](undefined#window-ID).
...
winrestview({dict})
		Uses the [Dictionary](undefined#Dictionary) returned by |winsaveview()| to restore
...
winsaveview()	Returns a [Dictionary](undefined#Dictionary) that contains information to restore
...
writefile({object}, {fname} [, {flags}])
		When {object} is a [List](undefined#List) write it to file {fname}.  Each list
		...
		When {object} is a [Blob](undefined#Blob) write the bytes to file {fname}
		unmodified.
...
[pattern](undefined#pattern) is normally used to find a match in the buffer lines.  When a

...
List		An ordered sequence of items, see [List](undefined#List) for details.
		Example: [1, 2, ['a', 'b']]
...
[expr9](undefined#expr9)	number			number constant
...
Using "is" or "isnot" with a [List](undefined#List), [Dictionary](undefined#Dictionary) or [Blob](undefined#Blob) checks whether
...
### <a id=":let-register :let-@" class="section-title" href="#:let-register :let-@">:let @{reg-name} = {expr1}</a>
			...
			[registers](undefined#registers)).  "@@" can be used for the unnamed
...
### <a id=":let-unpack E687 E688" class="section-title" href="#:let-unpack E687 E688">:let [{name1}, {name2}, ...] = {expr1}</a>
			{expr1} must evaluate to a [List](undefined#List).  The first item in
			the list is assigned to {name1}, the second item to
			{name2}, etc.
			The number of names must match the number of items in
			the [List](undefined#List).
...
:let [{name1}, {name2}, ...] -= {expr1}
			...
			[List](undefined#List) item.

### <a id="E452" class="section-title" href="#E452">:let [{name}, ..., ; {lastname}] = {expr1}</a>
			Like |:let-unpack| above, but the [List](undefined#List) may have more
...
text...
{endmarker}
			Set internal variable {var-name} to a [List](undefined#List)
			containing the lines of text bounded by the string
			{endmarker}. The lines of text is used as a
			[literal-string](undefined#literal-string).
...
### <a id=":unlet :unl E108 E795" class="section-title" href="#:unlet :unl E108 E795">:unl[et][!] {name} ...</a>
			...
			may also be a [List](undefined#List) or [Dictionary](undefined#Dictionary) item.
			With [!] no error message is given for non-existing
			variables.
			One or more items from a [List](undefined#List) can be removed:
```				:unlet list[3]	  " remove fourth item
				:unlet list[3:]   " remove fourth item to last
			One item from a [Dictionary](undefined#Dictionary) can be removed at a time:
				:unlet dict['two']
				:unlet dict.two
...
### <a id="E741 E940" class="section-title" href="#E741 E940">Note:</a>
			...
			[depth] is relevant when locking a [List](undefined#List) or
			[Dictionary](undefined#Dictionary).  It specifies how deep the locking goes:
				1	Lock the [List](undefined#List) or [Dictionary](undefined#Dictionary) itself,
					cannot add or remove items, but can
					still change their values.
				2	Also lock the values, cannot change
					the items.  If an item is a [List](undefined#List) or
					[Dictionary](undefined#Dictionary), cannot add or remove
					items, but can still change the
					values.
				3	Like 2 but for the [List](undefined#List) /
					[Dictionary](undefined#Dictionary) in the [List](undefined#List) /
					[Dictionary](undefined#Dictionary), one level deeper.
			The default [depth] is 2, thus when {name} is a [List](undefined#List)
			or [Dictionary](undefined#Dictionary) the values cannot be changed.
### <a id="E743" class="section-title" href="#E743">Note:</a>
			For unlimited depth use [!] and omit [depth].
			However, there is a maximum depth of 100 to catch
			loops.

			Note that when two variables refer to the same [List](undefined#List)
			and you lock one of them, the [List](undefined#List) will also be
...
### <a id=":endfo :endfor" class="section-title" href="#:endfo :endfor">:endfo[r]</a>
			...
			each item in {object}.  {object} can be a [List](undefined#List),
			a [Blob](undefined#Blob) or a [String](undefined#String).
...
```
			...
			When {object} is a [List](undefined#List) and not making a copy, Vim
			stores a reference to the next item in the [List](undefined#List)
			...
			to make a [List](undefined#List) empty):
```				for item in mylist
				   call remove(mylist, 0)
				endfor
			Note that reordering the [List](undefined#List) (e.g., with sort() or
			reverse()) may have unexpected effects.

			When {object} is a [Blob](undefined#Blob), Vim always makes a copy to
			iterate over.  Unlike with [List](undefined#List), modifying the
			[Blob](undefined#Blob) does not affect the iteration.

			When {object} is a [String](undefined#String) each item is a string with
...
```
			...
			|v:exception|.  Also see [throw-variables](undefined#throw-variables).
...
### <a id=":echo-self-refer" class="section-title" href="#:echo-self-refer">Note:</a>
			...
			"[...@level]" (self-referencing [List](undefined#List)) or
			"{...@level}" (self-referencing [Dict](undefined#Dict)):
		:let l = []
		:call add(l, l)
		:let l2 = []
		:call add(l2, [l2])
...
:echom[sg] {expr1} ..	Echo the expression(s) as a true message, saving the
			message in the [message-history](undefined#message-history).
...
:echoe[rr] {expr1} ..	Echo the expression(s) as an error message, saving the
			message in the [message-history](undefined#message-history).  When used in a
			script or function the line number will be added.
			Spaces are placed between the arguments as with the
			`:echomsg` command.  When used inside a try conditional,
			the message is raised as an error exception instead
			(see [try-echoerr](undefined#try-echoerr)).
			Example:
...
```		:execute '!ls' | echo "theend"
		...
		:execute "normal ixxx\<Esc>"
			This has an <Esc> character, see [expr-string](undefined#expr-string).
...
[catch-errors](undefined#catch-errors) and [catch-interrupt](undefined#catch-interrupt).  You can also explicitly throw an
exception by using the ":throw" command, see [throw-catch](undefined#throw-catch).
...
see [try-nesting](undefined#try-nesting).
...
exception might, however, be caught elsewhere, see [try-nesting](undefined#try-nesting).
   When during execution of the finally clause (if present) an exception is
thrown, the remaining lines in the finally clause are skipped.  If the finally
clause has been taken because of an exception from the try block or one of the
catch clauses, the original (pending) exception is discarded.  The commands
following the ":endtry" are not executed, and the exception from the finally
clause is propagated and can be caught elsewhere, see [try-nesting](undefined#try-nesting).
...
For examples see [throw-catch](undefined#throw-catch) and [try-finally](undefined#try-finally).
...
For examples see [throw-catch](undefined#throw-catch).
...
(see [debug-scripts](/neovim-docs-web/en/vim/repeat#debug-scripts)).
...
commands, see [try-conditionals](undefined#try-conditionals).   The values to be caught by each ":catch"
...
[catch-errors](undefined#catch-errors) and [ignore-errors](undefined#ignore-errors).
...
one (see [except-several-errors](undefined#except-several-errors)).  So you can catch it by
	:catch /^Vim(\a\+):E121:/
...
command on that line.  See [debug-scripts](/neovim-docs-web/en/vim/repeat#debug-scripts).
...
failed, if known.  See [catch-errors](undefined#catch-errors).
...
a "E600: Missing :endtry" error message is given, see [except-single-line](undefined#except-single-line).
...
These items are not allowed in the sandbox:
	...
	- setting certain options (see [option-summary](/neovim-docs-web/en/vim/options#option-summary))
...
*hl-NvimUnaryMinus*                  NvimUnaryOperator       [expr-unary--](undefined#expr-unary--)
*hl-NvimNot*                         NvimUnaryOperator       |expr-!|

*hl-NvimBinaryOperator*              NvimOperator            Generic binary op
*hl-NvimComparison*                  NvimBinaryOperator      Any [expr4](undefined#expr4) operator
*hl-NvimComparisonModifier*          NvimComparison          `#`/`?` near [expr4](undefined#expr4) op
*hl-NvimBinaryPlus*                  NvimBinaryOperator      |expr-+|
*hl-NvimBinaryMinus*                 NvimBinaryOperator      [expr--](undefined#expr--)
*hl-NvimConcat*                      NvimBinaryOperator      |expr-.|
*hl-NvimConcatOrSubscript*           NvimConcat              |expr-.| or [expr-entry](undefined#expr-entry)
*hl-NvimOr*                          NvimBinaryOperator      [expr-barbar](/neovim-docs-web/en/vim/eval#expr-barbar)
*hl-NvimAnd*                         NvimBinaryOperator      |expr-&&|
*hl-NvimMultiplication*              NvimBinaryOperator      [expr-star](undefined#expr-star)
*hl-NvimDivision*                    NvimBinaryOperator      |expr-/|
*hl-NvimMod*                         NvimBinaryOperator      |expr-%|

*hl-NvimTernary*                     NvimOperator            `?` in [expr1](undefined#expr1)
*hl-NvimTernaryColon*                NvimTernary             `:` in [expr1](undefined#expr1)

*hl-NvimParenthesis*                 Delimiter               Generic bracket
*hl-NvimLambda*                      NvimParenthesis         `{`/`}` in [lambda](undefined#lambda)
*hl-NvimNestingParenthesis*          NvimParenthesis         `(`/`)` in [expr-nesting](undefined#expr-nesting)
*hl-NvimCallingParenthesis*          NvimParenthesis         `(`/`)` in [expr-function](undefined#expr-function)
...
*hl-NvimDict*                        NvimContainer           `{`/`}` in [dict](undefined#dict) literal
*hl-NvimList*                        NvimContainer           `[`/`]` in [list](undefined#list) literal

*hl-NvimIdentifier*                  Identifier              Generic identifier
*hl-NvimIdentifierScope*             NvimIdentifier          Namespace: letter
                                                           before `:` in
                                                           [internal-variables](undefined#internal-variables)
...
*hl-NvimIdentifierKey*               NvimIdentifier          Identifier after
                                                           [expr-entry](undefined#expr-entry)

*hl-NvimColon*                       Delimiter               `:` in [dict](undefined#dict) literal
*hl-NvimComma*                       Delimiter               `,` in [dict](undefined#dict) or [list](undefined#list)
                                                           literal or
                                                           [expr-function](undefined#expr-function)
*hl-NvimArrow*                       Delimiter               `->` in [lambda](undefined#lambda)

*hl-NvimRegister*                    SpecialChar             [expr-register](undefined#expr-register)
*hl-NvimNumber*                      Number                  Non-prefix digits
                                                           in integer
                                                           [expr-number](undefined#expr-number)
*hl-NvimNumberPrefix*                Type                    `0` for [octal-number](undefined#octal-number)
                                                           `0x` for [hex-number](undefined#hex-number)
                                                           `0b` for [binary-number](undefined#binary-number)
*hl-NvimFloat*                       NvimNumber              Floating-point
                                                           number

*hl-NvimOptionSigil*                 Type                    `&` in [expr-option](undefined#expr-option)
...
*hl-NvimEnvironmentSigil*            NvimOptionSigil         `$` in [expr-env](undefined#expr-env)
...
*hl-NvimDoubleQuote*                 NvimStringQuote         `"` in [expr-quote](undefined#expr-quote)
*hl-NvimDoubleQuotedBody*            NvimStringBody          Literal part of
                                                           [expr-quote](undefined#expr-quote) body
*hl-NvimDoubleQuotedEscape*          NvimStringSpecial       Valid [expr-quote](undefined#expr-quote)
                                                           escape sequence
*hl-NvimDoubleQuotedUnknownEscape*   NvimInvalidValue        Unrecognized
                                                           [expr-quote](undefined#expr-quote) escape
                                                           sequence

...
'directory' 'dir'	string	(default "$XDG_STATE_HOME/nvim/swap//")
	...
	- Empty means that no swap file will be used (recovery is
	  impossible!) and no [E303](undefined#E303) error will be given.
...
'display' 'dy'		string	(default "lastline")
	...
	msgsep		Obsolete flag. Allowed but takes no effect. [msgsep](undefined#msgsep)

	When neither "lastline" nor "truncate" is included, a last line that
	doesn't fit is replaced with "@" lines.

	The "@" character can be changed by setting the "lastline" item in
	'fillchars'.  The character is highlighted with [hl-NonText](undefined#hl-NonText).
...
'encoding' 'enc'
	String-encoding used internally and for [RPC](undefined#RPC) communication.
	Always UTF-8.
...
'equalprg' 'ep'		string	(default "")
	...
	or 'indentexpr'.
	Environment variables are expanded |:set_env|.  See [option-backslash](undefined#option-backslash)
...
'errorfile' 'ef'	string	(default: "errors.err")
	...
	following argument.  See [-q](undefined#-q).
	NOT used for the ":make" command.  See 'makeef' for that.
	Environment variables are expanded |:set_env|.
	See [option-backslash](undefined#option-backslash) about including spaces and backslashes.
	This option cannot be set from a [modeline](undefined#modeline) or in the [sandbox](undefined#sandbox), for
	security reasons.
...
'errorformat' 'efm'	string	(default is very long)
	...
	(see [errorformat](undefined#errorformat)).
...
'expandtab' 'et'	boolean	(default off)
	...
	on, use CTRL-V<Tab>.  See also |:retab| and [ins-expandtab](undefined#ins-expandtab).
...
'fileencoding' 'fenc'	string (default: "")
	...
	See [encoding-names](undefined#encoding-names) for the possible values.  Additionally, values may be
	specified that can be handled by the converter, see
	[mbyte-conversion](undefined#mbyte-conversion).
	...
	[encoding-names](undefined#encoding-names), it is replaced by the standard name.  For example
...
#### <a id="au BufReadPost  if search('\S', 'w') == 0 |" class="section-title" href="#au BufReadPost  if search('\S', 'w') == 0 |">```</a>
	...
	recognized as "utf-8".  You can use the [8g8](undefined#8g8) command to find the
...
'fileformat' 'ff'	string (Windows default: "dos",
	...
	See [file-formats](undefined#file-formats) and [file-read](undefined#file-read).
...
'fileformats' 'ffs'	string (default:
	...
	- When 'fileformats' is set to one or more names, automatic detection
	  ...
	  used.
	Also see [file-formats](undefined#file-formats).
...
### <a id="/ vim: set filetype=idl : /" class="section-title" href="#/ vim: set filetype=idl : /">Note:</a>
	[FileType](undefined#FileType) [filetypes](undefined#filetypes)
...
'fillchars' 'fcs'	string	(default "")
			global or local to window [global-local](undefined#global-local)
	...
	The highlighting used for these items:
	  ...
	  stl		StatusLine		[hl-StatusLine](undefined#hl-StatusLine)
	  stlnc		StatusLineNC		[hl-StatusLineNC](undefined#hl-StatusLineNC)
	  wbr		WinBar			[hl-WinBar](undefined#hl-WinBar) or [hl-WinBarNC](undefined#hl-WinBarNC)
	  horiz		WinSeparator		[hl-WinSeparator](undefined#hl-WinSeparator)
	  horizup	WinSeparator		[hl-WinSeparator](undefined#hl-WinSeparator)
	  horizdown	WinSeparator		[hl-WinSeparator](undefined#hl-WinSeparator)
	  vert		WinSeparator		[hl-WinSeparator](undefined#hl-WinSeparator)
	  vertleft	WinSeparator		[hl-WinSeparator](undefined#hl-WinSeparator)
	  vertright	WinSeparator		[hl-WinSeparator](undefined#hl-WinSeparator)
	  verthoriz	WinSeparator		[hl-WinSeparator](undefined#hl-WinSeparator)
	  fold		Folded			[hl-Folded](undefined#hl-Folded)
	  diff		DiffDelete		[hl-DiffDelete](undefined#hl-DiffDelete)
	  eob		EndOfBuffer		[hl-EndOfBuffer](undefined#hl-EndOfBuffer)
	  lastline	NonText			[hl-NonText](undefined#hl-NonText)
...
'foldenable' 'fen'	boolean (default on)
	...
	with the [zi](undefined#zi) command.  The 'foldcolumn' will remain blank when
	'foldenable' is off.
...
'foldexpr' 'fde'	string (default: "0")
	...
	for each line to obtain its fold level.  See [fold-expr](undefined#fold-expr).

	The expression will be evaluated in the [sandbox](undefined#sandbox) if set from a
	modeline, see [sandbox-option](undefined#sandbox-option).
	This option can't be set from a [modeline](undefined#modeline) when the 'diff' option is
	on or the 'modelineexpr' option is off.

	It is not allowed to change text or jump to another window while
	evaluating 'foldexpr' [textlock](/neovim-docs-web/en/vim/eval#textlock).
...
'foldignore' 'fdi'	string (default: "#")
	...
	The default "#" works well for C programs.  See [fold-indent](undefined#fold-indent).
...
'foldlevel' 'fdl'	number (default: 0)
	...
	close fewer folds.
	This option is set by commands like [zm](undefined#zm), [zM](undefined#zM) and [zR](undefined#zR).
	See [fold-foldlevel](undefined#fold-foldlevel).
...
'foldlevelstart' 'fdls'	number (default: -1)
	...
	overrules this option.  Starting to edit a file for [diff-mode](undefined#diff-mode) also
...
'foldmarker' 'fmr'	string (default: "{{{,}}}")
	...
	See [fold-marker](undefined#fold-marker).

### <a id="'foldmethod' 'fdm'" class="section-title" href="#'foldmethod' 'fdm'">Note:</a>
'foldmethod' 'fdm'	string (default: "manual")
			local to window
	The kind of folding used for the current window.  Possible values:
	[fold-manual](undefined#fold-manual)	manual	    Folds are created manually.
	[fold-indent](undefined#fold-indent)	indent	    Lines with equal indent form a fold.
	[fold-expr](undefined#fold-expr)	expr	    'foldexpr' gives the fold level of a line.
	[fold-marker](undefined#fold-marker)	marker	    Markers are used to specify folds.
	[fold-syntax](undefined#fold-syntax)	syntax	    Syntax highlighting items specify folds.
	[fold-diff](undefined#fold-diff)	diff	    Fold text that is not changed.
...
'foldopen' 'fdo'	string (default: "block,hor,mark,percent,quickfix,
	...
	Add the [zv](undefined#zv) command to the mapping to get the same effect.
	...
	To close folds you can re-apply 'foldlevel' with the [zx](undefined#zx) command or
...
'foldtext' 'fdt'	string (default: "foldtext()")
	...
	fold.  See [fold-foldtext](undefined#fold-foldtext).
	...
	It is not allowed to change text or jump to another window while
	evaluating 'foldtext' [textlock](/neovim-docs-web/en/vim/eval#textlock).

### <a id="'formatexpr' 'fex'" class="section-title" href="#'formatexpr' 'fex'">Note:</a>
'formatexpr' 'fex'	string (default "")
			local to buffer
	Expression which is evaluated to format a range of lines for the [gq](undefined#gq)
	...
	autoload/mylang.vim file in 'runtimepath'. [autoload](undefined#autoload)
	...
	The expression will be evaluated in the [sandbox](undefined#sandbox) when set from a
	modeline, see [sandbox-option](undefined#sandbox-option).  That stops the option from working,
...
'formatoptions' 'fo'	string (default: "tcqj")
	...
	formatting is to be done.  See [fo-table](undefined#fo-table).  When the 'paste' option is
...
'formatprg' 'fp'	string (default "")
	...
	selected with the [gq](undefined#gq) operator.  The program must take the input on
	stdin and produce the output on stdout.  The Unix program "fmt" is
	such a program.
	If the 'formatexpr' option is not empty it will be used instead.
	Otherwise, if 'formatprg' option is an empty string, the internal
	format function will be used [C-indenting](/neovim-docs-web/en/vim/indent#C-indenting).
	Environment variables are expanded |:set_env|.  See [option-backslash](undefined#option-backslash)
	about including spaces and backslashes.
	This option cannot be set from a [modeline](undefined#modeline) or in the [sandbox](undefined#sandbox), for
	security reasons.
...
'fsync' 'fs'		boolean	(default off)
	...
	(|:write|, |writefile()|, …), [swap-file](/neovim-docs-web/en/vim/recover#swap-file) and [shada-file](undefined#shada-file). This
	...
	- [CursorHold](undefined#CursorHold) event is triggered
	- |:preserve| is called
...
'gdefault' 'gd'		boolean	(default off)
	...
	of all or one match.  See [complex-change](/neovim-docs-web/en/vim/change#complex-change).
...
'grepformat' 'gfm'	string	(default "%f:%l:%m,%f:%l%m,%f  %l%m")
	...
	'errorformat' option: see [errorformat](undefined#errorformat).
...
### <a id="The placeholder "$" is allowed to specify where the arguments" class="section-title" href="#The placeholder "$" is allowed to specify where the arguments">	line.</a>
	...
	[option-backslash](undefined#option-backslash) about including spaces and backslashes.
...
'guicursor' 'gcr'	string	(default "n-v-c-sm:block,i-ci-ve:ver25,r-cr-o:hor20")
	...
	terminals.  See [tui-cursor-shape](undefined#tui-cursor-shape).
	...
	The argument-list is a dash separated list of these arguments:
		...
		{group-name}
			...
			cursor.
			In the [TUI](undefined#TUI):
			- [inverse](undefined#inverse)/reverse and no group-name are interpreted
			  as "host-terminal default cursor colors" which
			  typically means "inverted bg and fg colors".
			- [ctermfg](undefined#ctermfg) and [guifg](undefined#guifg) are ignored.
		{group-name}/{group-name}
			Two highlight group names, the first is used when
			no language mappings are used, the other when they
			are. [language-mapping](undefined#language-mapping)
...
'guifont' 'gfn'		string	(default "")
	...
	[option-backslash](undefined#option-backslash).  For example:
...
### <a id="E236" class="section-title" href="#E236">Note:</a>
...
### <a id="'go-M'" class="section-title" href="#'go-M'">Note:</a>
	  'M'	The system menu "$VIMRUNTIME/menu.vim" is not sourced.  Note
		...
		switching on syntax or filetype recognition (when the [gvimrc](undefined#gvimrc)
...
### <a id="'go-b'" class="section-title" href="#'go-b'">Note:</a>
	  'b'	Bottom (horizontal) scrollbar is present.  Its size depends on
		...
		flag is included. [gui-horiz-scroll](undefined#gui-horiz-scroll)
### <a id="'go-h'" class="section-title" href="#'go-h'">Note:</a>
	  'h'	Limit horizontal scrollbar size to the length of the cursor
		line.  Reduces computations. [gui-horiz-scroll](undefined#gui-horiz-scroll)

	And yes, you may even have scrollbars on the left AND the right if
	you really want to :-).  See [gui-scrollbars](/neovim-docs-web/en/vim/gui#gui-scrollbars) for more information.
...
### <a id="'go-p'" class="section-title" href="#'go-p'">Note:</a>
	  'p'	Use Pointer callbacks for X11 GUI.  This is required for some
		...
		before starting the GUI.  Set it in your [gvimrc](undefined#gvimrc).  Adding or
...
'guitablabel' 'gtl'	string	(default empty)
	...
	default label.  See [setting-guitablabel](/neovim-docs-web/en/vim/tabpage#setting-guitablabel) for more info.
...
'helpfile' 'hf'		string	(default (MS-Windows) "$VIMRUNTIME\doc\help.txt"
	...
	tried.  Also see |$VIMRUNTIME| and [option-backslash](undefined#option-backslash) about including
...
```		:set helplang=de,it
	...
	See [help-translated](/neovim-docs-web/en/vim/helphelp#help-translated).
...
'hidden' 'hid'		boolean	(default on)
	...
	when it is [abandon](undefined#abandon)ed.  When on a buffer becomes hidden when it is
	[abandon](undefined#abandon)ed.  A buffer displayed in another window does not become
	hidden, of course.
	...
	- the '!' flag was used
	Also see [windows](undefined#windows).
...
'history' 'hi'		number	(default: 10000)
	...
	each of these histories (see [cmdline-editing](/neovim-docs-web/en/vim/cmdline#cmdline-editing)).
...
'hlsearch' 'hls'	boolean	(default on)
	...
	The [hl-Search](undefined#hl-Search) highlight group determines the highlighting for all
	matches not under the cursor while the [hl-CurSearch](undefined#hl-CurSearch) highlight group
	(if defined) determines the highlighting for the match under the
	cursor. If [hl-CurSearch](undefined#hl-CurSearch) is not defined, then [hl-Search](undefined#hl-Search) is used for
	...
	with the 'h' flag in 'shada' [shada-h](undefined#shada-h).
...
'inccommand' 'icm'	string	(default "nosplit")
	...
	[Command-line-mode](undefined#Command-line-mode) is done.
...
### <a id="string	(default "^\s#\sinclude")" class="section-title" href="#string	(default "^\s#\sinclude")">'include' 'inc'</a>
	...
	pattern, just like for the "/" command (See [pattern](undefined#pattern)).  The default
...
'includeexpr' 'inex'	string	(default "")
	...
	Also used for the [gf](undefined#gf) command if an unmodified file name can't be
	...
	It is not allowed to change text or jump to another window while
	evaluating 'includeexpr' [textlock](/neovim-docs-web/en/vim/eval#textlock).
...
'incsearch' 'is'	boolean	(default on)
	...
	The [hl-IncSearch](undefined#hl-IncSearch) highlight group determines the highlighting.
...
```		:set indentexpr=GetMyIndent()
	...
	"msg".
	See [indent-expression](/neovim-docs-web/en/vim/indent#indent-expression).
	...
	It is not allowed to change text or jump to another window while
	evaluating 'indentexpr' [textlock](/neovim-docs-web/en/vim/eval#textlock).
...
'indentkeys' 'indk'	string	(default "0{,0},0),0],:,0#,!^F,o,O,e")
	...
	The format is identical to 'cinkeys', see [indentkeys-format](undefined#indentkeys-format).
	See [C-indenting](/neovim-docs-web/en/vim/indent#C-indenting) and [indent-expression](/neovim-docs-web/en/vim/indent#indent-expression).

### <a id="'infercase' 'inf' 'noinfercase' 'noinf'" class="section-title" href="#'infercase' 'inf' 'noinfercase' 'noinf'">Note:</a>
'infercase' 'inf'	boolean	(default off)
			local to buffer
	When doing keyword completion in insert mode [ins-completion](/neovim-docs-web/en/vim/insert#ins-completion), and
...
'isfname' 'isf'		string	(default for Windows:
	...
	the tags file.  It is also used for "\f" in a [pattern](undefined#pattern).
...
'isident' 'isi'		string	(default for Windows:
	...
	[pattern](undefined#pattern).  See 'isfname' for a description of the format of this
...
'iskeyword' 'isk'	string (default: @,48-57,_,192-255)
	...
	"w", "*", "[i", etc.  It is also used for "\k" in a [pattern](undefined#pattern).  See
...
'isprint' 'isp'	string	(default: "@,161-255")
	...
	screen.  It is also used for "\p" in a [pattern](undefined#pattern).  The characters from
	...
	[hl-SpecialKey](undefined#hl-SpecialKey)
...
'jumpoptions' 'jop'	string	(default "")
			global
	List of words that change the behavior of the [jumplist](undefined#jumplist).
	  stack         Make the jumplist behave like the tagstack or like a
			...
			[jumplist-stack](undefined#jumplist-stack)

	  view          When moving through the jumplist, [changelist](undefined#changelist),
			[alternate-file](undefined#alternate-file) or using [mark-motions](undefined#mark-motions) try to
			restore the [mark-view](undefined#mark-view) in which the action occurred.
...
'keymap' 'kmp'		string	(default "")
			local to buffer
	Name of a keyboard mapping.  See [mbyte-keymap](/neovim-docs-web/en/vim/mbyte#mbyte-keymap).
...
'keywordprg' 'kp'	string	(default ":Man", Windows: ":help")
			global or local to buffer [global-local](undefined#global-local)
	Program to use for the [K](undefined#K) command.  Environment variables are
...
'laststatus' 'ls'	number	(default 2)
	...
	windows, but it takes another screen line. [status-line](undefined#status-line)
...
'lines'			number	(default 24 or terminal height)
	...
	When Vim is running in the GUI or in a resizable window, setting this
	option will cause the window size to be changed.  When you only want
	to use the size for the GUI, put the command in your [gvimrc](undefined#gvimrc) file.
...
### <a id="lcs-leadmultispace" class="section-title" href="#lcs-leadmultispace">Note:</a>
	  leadmultispace:c...
			Like the [lcs-multispace](undefined#lcs-multispace) value, but for leading
			spaces only.  Also overrides [lcs-lead](undefined#lcs-lead) for leading
			multiple spaces.
...
```
	...
	[hl-NonText](undefined#hl-NonText) highlighting will be used for "eol", "extends" and
	"precedes". [hl-Whitespace](undefined#hl-Whitespace) for "nbsp", "space", "tab", "multispace",
	"lead" and "trail".

### <a id="'lpl' 'nolpl' 'loadplugins' 'noloadplugins'" class="section-title" href="#'lpl' 'nolpl' 'loadplugins' 'noloadplugins'">Note:</a>
'loadplugins' 'lpl'	boolean	(default on)
			global
	When on the plugin scripts are loaded when starting up [load-plugins](undefined#load-plugins).
	This option can be reset in your [vimrc](undefined#vimrc) file to disable the loading
	of plugins.
	Note that using the "-u NONE" and "--noplugin" command line arguments
	reset this option. [-u](undefined#-u) [--noplugin](undefined#--noplugin)

### <a id="'magic' 'nomagic'" class="section-title" href="#'magic' 'nomagic'">Note:</a>
'magic'			boolean	(default on)
			global
	Changes the special characters that can be used in search patterns.
	See [pattern](undefined#pattern).
...
'matchpairs' 'mps'	string	(default "(:),{:},[:]")
	...
	the $VIMRUNTIME/plugin directory. [add-local-help](/neovim-docs-web/en/usr/usr_05#add-local-help)
...
'maxfuncdepth' 'mfd'	number	(default 100)
	...
	command recursion, see [E169](undefined#E169).
	See also |:function|.
...
'maxmapdepth' 'mmd'	number	(default 1000)
	...
	[key-mapping](undefined#key-mapping).
...
'mkspellmem' 'msm'	string	(default "460000,2000,500")
	...
	This option cannot be set from a [modeline](undefined#modeline) or in the [sandbox](undefined#sandbox).
...
'modeline' 'ml'		boolean	(default: on (off for root))
	...
	no lines are checked.  See [modeline](undefined#modeline).
...
'modelineexpr' 'mle'	boolean (default: off)
	...
	'modelineexpr'.  Also see [modeline](undefined#modeline).
	This option cannot be set from a [modeline](undefined#modeline) or in the [sandbox](undefined#sandbox), for
	security reasons.
...
'modifiable' 'ma'	boolean	(default on)
	...
	Can be reset on startup with the [-M](undefined#-M) command line argument.
...
'modified' 'mod'	boolean	(default off)
	...
	1. A change was made to the text since it was last written.  Using the
	   [undo](undefined#undo) command to go back to the original text will reset the
	...
	FileAppendPost or VimLeave autocommand event.  See [gzip-example](undefined#gzip-example) for
	an explanation.
...
'more'			boolean	(default: on)
	...
	the [more-prompt](undefined#more-prompt).  When this option is off there are no pauses, the
...
```
	...
	Mouse support can be enabled for different modes:
		...
		r	for [hit-enter](undefined#hit-enter) and [more-prompt](undefined#more-prompt) prompt

	Left-click anywhere in a text buffer to place the cursor there.  This
	works with operators too, e.g. type [d](undefined#d) then left-click to delete text
	from the current cursor position to the position where you clicked.

	Drag the [status-line](undefined#status-line) or vertical separator of a window to resize it.
	...
	For scrolling with a mouse wheel see [scroll-mouse-wheel](/neovim-docs-web/en/vim/scroll#scroll-mouse-wheel).
...
'mousemodel' 'mousem'	string	(default "popup_setpos")
	...
	Nvim creates a default [popup-menu](undefined#popup-menu) but you can redefine it.

	Note that you can further refine the meaning of buttons with mappings.
	See [mouse-overview](undefined#mouse-overview).  But mappings are NOT used for modeless selection.
...
'nrformats' 'nf'	string	(default "bin,hex")
	...
	respectively; see [CTRL-A](undefined#CTRL-A) for more info on these commands.
...
'number' 'nu'		boolean	(default off)
	...
	For highlighting see [hl-LineNr](undefined#hl-LineNr), [hl-CursorLineNr](undefined#hl-CursorLineNr), and the
...
'operatorfunc' 'opfunc'	string	(default: empty)
	...
	the name of a function, a [lambda](undefined#lambda) or a [Funcref](undefined#Funcref). See
	[option-value-function](undefined#option-value-function) for more information.
...
'packpath' 'pp'		string	(default: see 'runtimepath')
	Directories used to find packages.  See [packages](/neovim-docs-web/en/vim/repeat#packages) and [rtp-packages](undefined#rtp-packages).
...
'paragraphs' 'para'	string	(default "IPLPPPQPP TPHPLIPpLpItpplpipbp")
	...
	of two letters (see [object-motions](/neovim-docs-web/en/vim/motion#object-motions)).

### <a id="'paste' 'nopaste'" class="section-title" href="#'paste' 'nopaste'">Note:</a>
'paste'			boolean	(default off)
			global
	This option is obsolete; [bracketed-paste-mode](undefined#bracketed-paste-mode) is built-in.
...
'patchexpr' 'pex'	string	(default "")
	...
	the resulting new version of the file.  See [diff-patchexpr](undefined#diff-patchexpr).
...
'path' 'pa'		string	(default on Unix: ".,/usr/include,,"
			...
			global or local to buffer [global-local](undefined#global-local)
	This is a list of directories which will be searched when using the
	[gf](undefined#gf), [f, ]f, ^Wf, |:find|, |:sfind|, |:tabfind| and other commands,
	...
	- Search upwards and downwards in a directory tree using "*", "**" and
	  ";".  See [file-searching](/neovim-docs-web/en/vim/editing#file-searching) for info and syntax.
...
'printdevice' 'pdev'	string	(default empty)
	...
	See [pdev-option](undefined#pdev-option).
	This option cannot be set from a [modeline](undefined#modeline) or in the [sandbox](undefined#sandbox), for
	security reasons.
...
'pumheight' 'ph'	number	(default 0)
	...
	([ins-completion-menu](undefined#ins-completion-menu)). Zero means "use available screen space".

### <a id="'pumwidth' 'pw'" class="section-title" href="#'pumwidth' 'pw'">Note:</a>
'pumwidth' 'pw'		number	(default 15)
			global
	Minimum width for the popup menu ([ins-completion-menu](undefined#ins-completion-menu)).  If the
...
'quickfixtextfunc' 'qftf'	string (default "")
	...
	[quickfix-window-function](/neovim-docs-web/en/vim/quickfix#quickfix-window-function) for an explanation of how to write the
	function and an example.  The value can be the name of a function, a
	[lambda](undefined#lambda) or a [Funcref](undefined#Funcref). See [option-value-function](undefined#option-value-function) for more
	information.
...
'redrawtime' 'rdt'	number	(default 2000)
	...
	limit syntax highlighting is disabled until [CTRL-L](undefined#CTRL-L) is used.
...
'regexpengine' 're'	number	(default 0)
			global
	This selects the default regexp engine. [two-engines](undefined#two-engines)
...
'relativenumber' 'rnu'	boolean	(default off)
	...
	each line. Relative line numbers help you use the [count](undefined#count) you can
	...
	When a long, wrapped line doesn't start with the first character, '-'
	characters are put before the number.
	See [hl-LineNr](undefined#hl-LineNr)  and [hl-CursorLineNr](undefined#hl-CursorLineNr) for the highlighting used for
	the number.
...
'revins' 'ri'		boolean	(default off)
	...
	backwards" [ins-reverse](undefined#ins-reverse).  This option can be toggled with the CTRL-_
...
'runtimepath' 'rtp'	string	(default:     "$XDG_CONFIG_HOME/nvim,
	...
	List of directories to be searched for these runtime files:
	  filetype.lua	filetypes [new-filetype](undefined#new-filetype)
	  autoload/	automatically loaded scripts [autoload-functions](/neovim-docs-web/en/vim/userfunc#autoload-functions)
	  colors/	color scheme files |:colorscheme|
	  compiler/	compiler files |:compiler|
	  doc/		documentation [write-local-help](undefined#write-local-help)
	  ftplugin/	filetype plugins [write-filetype-plugin](undefined#write-filetype-plugin)
	  indent/	indent scripts [indent-expression](/neovim-docs-web/en/vim/indent#indent-expression)
	  keymap/	key mapping files [mbyte-keymap](/neovim-docs-web/en/vim/mbyte#mbyte-keymap)
	  lang/		menu translations |:menutrans|
	  lua/		[Lua](undefined#Lua) plugins
	  menu.vim	GUI menus |menu.vim|
	  pack/		packages |:packadd|
	  parser/	[treesitter](undefined#treesitter) syntax parsers
	  plugin/	plugin scripts [write-plugin](/neovim-docs-web/en/usr/usr_41#write-plugin)
	  print/	files for printing [postscript-print-encoding](undefined#postscript-print-encoding)
	  query/	[treesitter](undefined#treesitter) queries
	  rplugin/	[remote-plugin](undefined#remote-plugin) scripts
	  spell/	spell checking files [spell](undefined#spell)
	  syntax/	syntax files [mysyntaxfile](undefined#mysyntaxfile)
	...
	2. Directories which must contain configuration files according to 
	   [xdg](undefined#xdg) ($XDG_CONFIG_DIRS, defaults to /etc/xdg).  This also contains
...
### <a id="packages-runtimepath" class="section-title" href="#packages-runtimepath">Note:</a>
	"start" packages will also be searched ([runtime-search-path](undefined#runtime-search-path)) for
...
```		:set runtimepath=~/vimruntime,/mygroup/vim,$VIMRUNTIME
	...
	With [--clean](undefined#--clean) the home directory entries are not included.
...
'scroll' 'scr'		number	(default: half the window height)
	...
	changes.  This may happen when enabling the [status-line](undefined#status-line) or
...
'scrollback' 'scbk'	number	(default: 10000)
	...
	Only in [terminal](undefined#terminal) buffers.

### <a id="'scrollbind' 'scb' 'noscrollbind' 'noscb'" class="section-title" href="#'scrollbind' 'scb' 'noscrollbind' 'noscb'">Note:</a>
'scrollbind' 'scb'	boolean  (default off)
			local to window
	See also [scroll-binding](/neovim-docs-web/en/vim/scroll#scroll-binding).  When this option is set, the current
...
'scrollopt' 'sbo'	string	(default "ver,jump")
	...
	Also see [scroll-binding](/neovim-docs-web/en/vim/scroll#scroll-binding).
...
'sections' 'sect'	string	(default "SHNHH HUnhsh")
	...
	two letters (See [object-motions](/neovim-docs-web/en/vim/motion#object-motions)).  The default makes a section start
...
'selectmode' 'slm'	string	(default "")
	...
	See [Select-mode](undefined#Select-mode).
...
'sessionoptions' 'ssop'	string	(default: "blank,buffers,curdir,folds,
	...
	something:
	   ...
	   slash	[deprecated](undefined#deprecated) Always enabled. Uses "/" in filenames.
	   unix		[deprecated](undefined#deprecated) Always enabled. Uses "\n" line endings.
...
'shada' 'sd'		string	(default for
	...
	when exiting Vim (see [shada-file](undefined#shada-file)).  The string should be a comma-
...
### <a id="shada-%" class="section-title" href="#shada-%">Note:</a>
	%	When included, save and restore the buffer list.  If Vim is
		...
		removable media ([shada-r](undefined#shada-r)) are not saved.
...
### <a id="shada-'" class="section-title" href="#shada-'">Note:</a>
	'	Maximum number of previously edited files for which the marks
		...
		'shada' is non-empty.
		Including this item also means that the [jumplist](undefined#jumplist) and the
		[changelist](undefined#changelist) are stored in the shada file.
### <a id="shada-/" class="section-title" href="#shada-/">Note:</a>
...
'shadafile' 'sdf'	string	(default: "")
			global
	When non-empty, overrides the file name used for [shada](undefined#shada) (viminfo).
	When equal to "NONE" no shada file will be read or written.
	This option can be set with the [-i](undefined#-i) command line flag.  The [--clean](undefined#--clean)
...
'shellcmdflag' 'shcf'	string	(default: "-c"; Windows: "/s /c")
	...
	See [option-backslash](undefined#option-backslash) about including spaces and backslashes.
	See [shell-unquoting](undefined#shell-unquoting) which talks about separating this option into 
	multiple arguments.
	This option cannot be set from a [modeline](undefined#modeline) or in the [sandbox](undefined#sandbox), for
	security reasons.
...
'shellpipe' 'sp'	string	(default ">", ">%s 2>&1", "| tee", "|& tee" or
	...
	error file.  See also |:make_makeprg|.  See [option-backslash](undefined#option-backslash) about
...
'shellredir' 'srr'	string	(default ">", ">&" or ">%s 2>&1")
	...
	file.  See also |:!|.  See [option-backslash](undefined#option-backslash) about including spaces
	and backslashes.
	The name of the temporary file can be represented by "%s" if necessary
	(the file name is appended automatically if no %s appears in the value
	of this option).
...
'shelltemp' 'stmp'	boolean	(default on)
	...
	can be detected.
	The [FilterReadPre](undefined#FilterReadPre), [FilterReadPost](undefined#FilterReadPost) and [FilterWritePre](undefined#FilterWritePre),
	[FilterWritePost](undefined#FilterWritePost) autocommands event are not triggered when
	'shelltemp' is off.
...
'shortmess' 'shm'	string	(default "filnxtToOF")
			global
	This option helps to avoid all the [hit-enter](undefined#hit-enter) prompts caused by file
	...
	It is a list of flags:
	 flag	meaning when present	~
	  ...
	  c	don't give [ins-completion-menu](undefined#ins-completion-menu) messages.  For example,
...
```		:set showbreak=>\ 
	...
	The [hl-NonText](undefined#hl-NonText) highlight group determines the highlighting.
...
'showfulltag' 'sft'	boolean (default off)
			global
	When completing a word in insert mode (see [ins-completion](/neovim-docs-web/en/vim/insert#ins-completion)) from the
...
'showmode' 'smd'	boolean	(default: on)
	...
	The [hl-ModeMsg](undefined#hl-ModeMsg) highlight group determines the highlighting.
	The option has no effect when 'cmdheight' is zero.
...
'showtabline' 'stal'	number	(default 1)
	...
	line.
	See [tab-page](undefined#tab-page) for more information about tab pages.
...
'smartindent' 'si'	boolean	(default off)
	...
	see [C-indenting](/neovim-docs-web/en/vim/indent#C-indenting).  When 'cindent' is on or 'indentexpr' is set,
...
'smarttab' 'sta'	boolean	(default on)
	...
	right [shift-left-right](undefined#shift-left-right).
	What gets inserted (a <Tab> or spaces) depends on the 'expandtab'
	option.  Also see [ins-expandtab](undefined#ins-expandtab).  When 'expandtab' is not set, the
...
'softtabstop' 'sts'	number	(default 0)
	...
	when 'paste' is reset.
	See also [ins-expandtab](undefined#ins-expandtab).  When 'expandtab' is not set, the number of
...
'spell'			boolean	(default off)
			local to window
	When on spell checking will be done.  See [spell](undefined#spell).
...
'spellcapcheck' 'spc'	string	(default "[.?!]\_[\])'" \t]\+")
	...
	with SpellCap [hl-SpellCap](undefined#hl-SpellCap) (unless the word is also badly spelled).
	When this check is not wanted make this option empty.
	Only used when 'spell' is set.
	Be careful with special characters, see [option-backslash](undefined#option-backslash) about
	including spaces and backslashes.
	To set this option automatically depending on the language, see
	[set-spc-auto](undefined#set-spc-auto).

### <a id="'spellfile' 'spf'" class="section-title" href="#'spellfile' 'spf'">Note:</a>
'spellfile' 'spf'	string	(default empty)
			local to buffer
	Name of the word list file where words are added for the [zg](undefined#zg) and [zw](undefined#zw)
	commands.  It must end in ".{encoding}.add".  You need to include the
	path, otherwise the file is placed in the current directory.
### <a id="E765" class="section-title" href="#E765">Note:</a>
	It may also be a comma-separated list of names.  A count before the
	[zg](undefined#zg) and [zw](undefined#zw) commands can be used to access each.  This allows using
...
### <a id="E757" class="section-title" href="#E757">Note:</a>
	...
	How the related spell files are found is explained here: [spell-load](undefined#spell-load).
	...
	not a dash.  Also see [set-spc-auto](undefined#set-spc-auto).
...
'statusline' 'stl'	string	(default empty)
	...
	Also see [status-line](undefined#status-line).
...
```		:set statusline=%!MyStatusLine()
	The *g:statusline_winid* variable will be set to the [window-ID](undefined#window-ID) of the
	...
	'laststatus' is 2 or 3) is controlling the output of [CTRL-G](undefined#CTRL-G).
	...
	p N   Percentage through file in lines as in [CTRL-G](undefined#CTRL-G).
...
### <a id="stl-%{ g:actual_curbuf g:actual_curwin" class="section-title" href="#stl-%{ g:actual_curbuf g:actual_curwin">Note:</a>
	...
	real current buffer and "g:actual_curwin" to the [window-ID](undefined#window-ID) of the
	real current window.  These values are strings.

	The 'statusline' option will be evaluated in the [sandbox](undefined#sandbox) if set from
	a modeline, see [sandbox-option](undefined#sandbox-option).
	This option cannot be set in a modeline when 'modelineexpr' is off.

	It is not allowed to change text or jump to another window while
	evaluating 'statusline' [textlock](/neovim-docs-web/en/vim/eval#textlock).
...
'suffixes' 'su'		string	(default ".bak,~,.o,.h,.info,.swp,.obj")
	...
	match a wildcard.  See [suffixes](undefined#suffixes).  Commas can be used to separate the
	suffixes.  Spaces after the comma are ignored.  A dot is also seen as
	the start of a suffix.  To avoid a dot or comma being recognized as a
	separator, precede it with a backslash (see [option-backslash](undefined#option-backslash) about
	including spaces and backslashes).
...
'swapfile' 'swf'	boolean (default on)
	...
	Also see [swap-file](/neovim-docs-web/en/vim/recover#swap-file).
...
'switchbuf' 'swb'	string	(default "uselast")
	...
	Mostly for [quickfix](undefined#quickfix) commands some values are also used for other
	commands, as mentioned below.
	Possible values (comma-separated list):
	   useopen	If included, jump to the first open window that
			contains the specified buffer (if there is one).
			Otherwise: Do not examine other windows.
			This setting is checked with [quickfix](undefined#quickfix) commands, when
	   ...
	   split	If included, split the current window before loading
			a buffer for a [quickfix](undefined#quickfix) command that display errors.
	   ...
	   uselast	If included, jump to the previously used window when
			jumping to errors with [quickfix](undefined#quickfix) commands.
...
#### <a id="/ vim: set syntax=idl : /" class="section-title" href="#/ vim: set syntax=idl : /">```</a>
	When a dot appears in the value then this separates two filetype
	names.  Example:
...
'tabline' 'tal'		string	(default empty)
	...
	tab pages line.  See [setting-tabline](/neovim-docs-web/en/vim/tabpage#setting-tabline) for more info.
...
'tabpagemax' 'tpm'	number	(default 50)
			global
	Maximum number of tab pages to be opened by the [-p](undefined#-p) command line
	argument or the ":tab all" command. [tabpage](undefined#tabpage)
...
'tabstop' 'ts'		number	(default 8)
	...
	3. Set 'tabstop' and 'shiftwidth' to whatever you prefer and use a
	   [modeline](undefined#modeline) to set these values when editing the file again.  Only
...
'tagfunc' 'tfu'		string	(default: empty)
	...
	tags.  See [tag-function](/neovim-docs-web/en/vim/tagsrch#tag-function) for an explanation of how to write the
...
'tags' 'tag'		string	(default "./tags;,tags")
	...
	[tags-option](undefined#tags-option).
	"*", "**" and other wildcards can be used to search for tags files in
	a directory tree.  See [file-searching](/neovim-docs-web/en/vim/editing#file-searching).  E.g., "/lib/**/tags" will
...
'tagstack' 'tgst'	boolean	(default on)
			global
	When on, the [tagstack](undefined#tagstack) is used normally.  When off, a ":tag" or
...
'termguicolors' 'tgc'	boolean (default off)
			global
	Enables 24-bit RGB color in the [TUI](undefined#TUI).  Uses "gui" |:highlight|
	attributes instead of "cterm" attributes. [guifg](undefined#guifg)
...
'textwidth' 'tw'	number	(default 0)
	...
	'formatoptions' and [ins-textwidth](/neovim-docs-web/en/vim/insert#ins-textwidth).
...
'thesaurus' 'tsr'	string	(default "")
	...
	[compl-thesaurus](undefined#compl-thesaurus).
	...
	To include a comma in a file name precede it with a backslash.  Spaces
	after a comma are ignored, otherwise spaces are included in the file
	name.  See [option-backslash](undefined#option-backslash) about using backslashes.  The use of
...
'thesaurusfunc' 'tsrfu'	string	(default: empty)
	...
	with CTRL-X CTRL-T. |i_CTRL-X_CTRL-T| See [compl-thesaurusfunc](undefined#compl-thesaurusfunc).
...
'ttimeout'		boolean (default on)
	...
	key code sequence has been received by the [TUI](undefined#TUI).
...
'undodir' 'udir'	string	(default "$XDG_STATE_HOME/nvim/undo//")
	...
	See [undo-persistence](undefined#undo-persistence).
	This option cannot be set from a [modeline](undefined#modeline) or in the [sandbox](undefined#sandbox), for
	security reasons.
...
'undofile' 'udf'	boolean	(default off)
	...
	For more information about this feature see [undo-persistence](undefined#undo-persistence).
...
'undolevels' 'ul'	number	(default 1000)
	...
	Also see [undo-two-ways](/neovim-docs-web/en/vim/undo#undo-two-ways).
	...
	Also see [clear-undo](undefined#clear-undo).
...
'undoreload' 'ur'	number	(default 10000)
	...
	Vim. [FileChangedShell](undefined#FileChangedShell)
...
'updatecount' 'uc'	number	(default: 200)
	...
	recovery [crash-recovery](undefined#crash-recovery)).  'updatecount' is set to zero by starting
	Vim with the "-n" option, see [startup](undefined#startup).  When editing in readonly
...
'updatetime' 'ut'	number	(default 4000)
	...
	written to disk (see [crash-recovery](undefined#crash-recovery)).  Also used for the
	[CursorHold](undefined#CursorHold) autocommand event.
...
'verbose' 'vbs'		number	(default 0)
			global
	Sets the verbosity level.  Also set by [-V](undefined#-V) and |:verbose|.
	...
	2	When a file is ":source"'ed, or [shada](undefined#shada) file is read or written.
...
'virtualedit' 've'	string	(default "")
	...
	break some plugins or Vim scripts.  For example because [l](undefined#l) can move
...
'wildchar' 'wc'		number	(default: <Tab>)
	...
	More info here: [cmdline-completion](/neovim-docs-web/en/vim/cmdline#cmdline-completion).
...
'wildcharm' 'wcm'	number	(default: none (0))
	...
	keys suitable for this option by looking at [ex-edit-index](/neovim-docs-web/en/vim/index#ex-edit-index).  Normally
...
'wildignore' 'wig'	string	(default "")
	...
	patterns is ignored when expanding [wildcards](undefined#wildcards), completing file or
...
```
	...
	[hl-WildMenu](undefined#hl-WildMenu) highlights the current match.
...
'wildoptions' 'wop'	string	(default "pum,tagfile")
			global
	A list of words that change how [cmdline-completion](/neovim-docs-web/en/vim/cmdline#cmdline-completion) is done.
	The following values are supported:
	  pum		Display the completion matches using the popup menu
			in the same style as the [ins-completion-menu](undefined#ins-completion-menu).
...
'window' 'wi'		number  (default screen height - 1)
			global
	Window height used for [CTRL-F](undefined#CTRL-F) and [CTRL-B](undefined#CTRL-B) when there is only one
...
'winheight' 'wh'	number	(default 1)
	...
	using the [VimEnter](undefined#VimEnter) event:
...
'winhighlight' 'winhl'	string (default empty)
	...
	[group-name](undefined#group-name) pairs "{hl-from}:{hl-to},..." where each {hl-from} is
	a [highlight-groups](undefined#highlight-groups) item to be overridden by {hl-to} group in
	the window.
...
'winfixheight' 'wfh'	boolean	(default off)
	...
	[preview-window](/neovim-docs-web/en/vim/windows#preview-window) and [quickfix-window](/neovim-docs-web/en/vim/quickfix#quickfix-window).
...
'wrap'			boolean	(default on)
	...
	See 'sidescroll', 'listchars' and [wrap-off](undefined#wrap-off).
	This option can't be set from a [modeline](undefined#modeline) when the 'diff' option is
	on.
...
'wrapmargin' 'wm'	number	(default 0)
	...
	See also 'formatoptions' and [ins-textwidth](/neovim-docs-web/en/vim/insert#ins-textwidth).
...
'write'			boolean	(default on)
	...
	still allowed.  Can be reset with the [-m](undefined#-m) or [-M](undefined#-M) command line
...
'writebackup' 'wb'	boolean	(default on)
	...
	See [backup-table](undefined#backup-table) for another explanation.
	When the 'backupskip' pattern matches, a backup is not made anyway.

...
It is also possible to use curly braces, see [curly-braces-names](/neovim-docs-web/en/vim/eval#curly-braces-names).

The [autoload](undefined#autoload) facility is useful to define a function only when it's called.
...
:fu[nction][!] {name}	List function {name}, annotated with line numbers
			unless "!" is given.
			{name} may be a [Dictionary](undefined#Dictionary) [Funcref](undefined#Funcref) entry:
```				:function dict.init
...
:fu[nction][!] {name}([arguments]) [range] [abort] [dict] [closure]
			...
			{name} can also be a [Dictionary](undefined#Dictionary) entry that is a
			[Funcref](undefined#Funcref):
```				:function dict.init(arg)
			"dict" must be an existing dictionary.  The entry
			"init" is added if it didn't exist yet.  Otherwise [!]
			is required to overwrite an existing function.  The
			result is a [Funcref](undefined#Funcref) to a numbered function.  The
			function can only be used with a [Funcref](undefined#Funcref) and will be
...
### <a id="E127 E122" class="section-title" href="#E127 E122">Note:</a>
			...
			For the {arguments} see [function-argument](undefined#function-argument).
...
### <a id=":func-range a:firstline a:lastline" class="section-title" href="#:func-range a:firstline a:lastline">Note:</a>
			...
			of each line.  See [function-range-example](undefined#function-range-example).
...
### <a id=":func-dict" class="section-title" href="#:func-dict">Note:</a>
			...
			be invoked through an entry in a [Dictionary](undefined#Dictionary).  The
			local variable "self" will then be set to the
			dictionary.  See [Dictionary-function](undefined#Dictionary-function).
...
can be 0).  "a:000" is set to a [List](undefined#List) that contains these arguments.  Note
that "a:1" is the same as "a:000[0]".
### <a id="E742" class="section-title" href="#E742">Note:</a>
The a: scope and the variables in it cannot be changed, they are fixed.
However, if a composite type is used, such as [List](undefined#List) or [Dictionary](undefined#Dictionary) , you can
change their contents.  Thus you can pass a [List](undefined#List) to a function and have the
function add an item to it.  If you want to make sure the function cannot
change a [List](undefined#List) or [Dictionary](undefined#Dictionary) use |:lockvar|.
...
lambda expressions [expr-lambda](undefined#expr-lambda).
...
To return more than one value, return a [List](undefined#List):
...
"BufNet".  Also see [FuncUndefined](undefined#FuncUndefined).
...
to allow calling packages' functions from your [vimrc](undefined#vimrc) when the packages have
not been added to 'runtimepath' yet (see [packages](/neovim-docs-web/en/vim/repeat#packages)).
...
Hint: If you distribute a bunch of scripts read [distribute-script](/neovim-docs-web/en/usr/usr_41#distribute-script).
